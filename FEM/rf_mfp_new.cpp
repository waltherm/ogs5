...
std::ios::pos_type CFluidProperties::Read(std::ifstream* mfp_file)
{
	...
	while (!new_keyword)
	{
		...
		if (line_string.find("$DENSITY") != string::npos)
		{
			...
			/// calculations
			/// The densites depend on the thermodynamic data base (including
			/// pressure and temperature corrections)
			/// in addition various gas/fluid models (&EOS) are defined in
			/// GEMS3K
			if (density_model ==
			                          ...
			                          // FiniteElement::TNEQ)
			{
				std::cout << "Warning: This density model requires two "
				             "components and their molar masses defined in the "
				             "mcp file!\n";
				density_pcs_name_vector.push_back("PRESSURE1");
				density_pcs_name_vector.push_back("TEMPERATURE1");
				density_pcs_name_vector.push_back("CONCENTRATION1");
		...
		if (line_string.find("$HEAT_CONDUCTIVITY") != string::npos)
		{
			         ...
			         // FiniteElement::TNEQ)
			{
				std::cout << "Warning: This heat conductivity model requires "
				             "two components and their molar masses defined in "
				             "the mcp file!\n";
				heat_conductivity_pcs_name_vector.push_back("PRESSURE1");
				heat_conductivity_pcs_name_vector.push_back("TEMPERATURE1");
	...
	if (variables)  // This condition is added by WW
	{
		...
		switch (density_model)
		{
			...
			case 19:  // KG44 get the density from GEMS calculations
				...
				// take values from last GEMS calculation ---> update during
				// iterations is not possible
	...
	else
	{
		...
		switch (density_model)
		{
			...
			case 7:  // rho_w^l(p,T) for gas phase
				...
				MFPCalcVapourPressure(primary_variable[0]);
				    air_gas_density = (MolarMass::Air *
			...
			case 19:  // KG44 get the density from GEMS calculations
				...
				{
					long elem =
					    Fem_Ele_Std->GetMeshElement()
					        ->GetIndex();  // kg44 need element index or node
					                       // index for GEMS ...currently we
					                       // stick to arithmetric average of
					                       // node data (even for gauss points!)
					density = m_vec_GEM->REACT_GEM::FluidDensity(
...
double CFluidProperties::Viscosity(double* variables)
{
	...
	switch (viscosity_model)
	{
		...
		case 26:  // Wilke (see Poling, B. E.; Prausnitz, J. M.; John Paul, O. &
			      // Reid, R. C. The properties of gases and liquids McGraw-Hill
			      // New York, 2001, 5: page 9.21)
			{
				double x[2], Vs[2];
...
double CFluidProperties::SpecificHeatCapacity(double* variables)
{
	...
	switch (heat_capacity_model)
	{
		...
		case 11:  // mole fraction weighted average of molar isochoric specific
			      // heat capacities converted into isobaric mixture specific
			      // heat capacity (ideal conversion)
			{
				// reactive component
				const double M1 = cp_vec[0]->molar_mass;
				const double M2 = cp_vec[1]->molar_mass;
...
double CFluidProperties::HeatConductivity(double* variables)
{
	...
	switch (heat_conductivity_model)
	{
		...
		case 11:  // Wassilijewa, Maso&Saxena (see Poling, B. E.; Prausnitz, J.
			      // M.; John Paul, O. & Reid, R. C. The properties of gases and
			      // liquids McGraw-Hill New York, 2001, 5: page 10.30f.)
			{
				double x[2], k[2];
...
double CFluidProperties::Enthalpy(int comp, double temperature)
{
	...
	if ((phase == 0) && (comp == 0))
		enthalpy = 733.0 * temperature +
		           (FluidConstant::GasConstant() * (temperature + 0.0)) /
                                                      ...
                                                      double* gp, double theta)
{
	...
	vapour_pressure = MFPCalcVapourPressure(temperature);
	dvapour_pressure_dT =
	    FluidConstant::ComponentMolarMassWater() * Enthalpy(comp, temperature) /
	    (FluidConstant::GasConstant() * temperature * temperature) *
	    vapour_pressure;

	drho_dT = -1.0 * FluidConstant::ComponentMolarMassWater() /
	          (FluidConstant::GasConstant() * temperature) *
		...
		case 15:  // volume translated Peng-Robinson
			if (eos_name == "VTPR" || eos_name == "PR" || eos_name == "IDEAL")
			{
				...
				for (int CIndex = 2; CIndex < cmpN + 2; CIndex++)
				{
					...
					fctA = v + c - b;
					fctC = v + c + b;
					dvdp = fctA * fctB / (p * fctB + 2.0 * p * fctA * fctC -
					                      2.0 * R * T * fctC + a);
					beta = dvdp;
					beta_m += beta;
					v_m += v;
