	...
	if (pcs_rwpt_count == 0)  // HS, no random walk detected.
	{
		...
		else
		{
			...
			for (i = 0; i < pcs_vector.size(); i++)
			{
				...
				if (pcs_vector[i]->getProcessType() == FiniteElement::TNEQ ||
					...
					std::cout << "Warning! For TNEQ/TES, the component order "
					             "in the mcp file needs to be INERT, "
					             "REACTIVE!\n";
			}
		}  // end of else
		...
		if (pcs_vector[0]->getProcessType() == FiniteElement::TNEQ ||
			...
			for (i = 0; i < cp_vec.size(); i++)
			{
				...
				{
					cp_vec[i]->fluid_id = 5;
					std::cout << "Warning: The thermal conductivity critical "
					             "enhancement model for oxygen has not been "
					             "implemented. Dilute and residual parts "
					             "only.\n";
				}
				else
					std::cout
 ...
 */
		           ...
		           outside this function  */
		{
			                                   ...
			                                   // coefficients
			{
				...
				// phase ....tr_phase=10
				dummy *= saturation;  // account for "mean element" saturation
				                      // in Archies law ....better would be to
				                      // get node saturations!
				...
				dummy = m_vec_GEM->REACT_GEM::GetNodePorosityValueInitial(
				    m_Elem->GetNodeIndex(i));  // ATTENTION: This does not
				                               // include initial saturation
				                               // different from 1
			...
			if (flowflag == 3)  // RICHARDS_FLOW ...get node based saturations
			{
				saturation = PCSGetEleMeanNodeSecondary_2(
				    ...
				    m_pcs->flow_pcs_type,
				    "SATURATION1",
				    1);  // this line accounts for element saturatuions for all
				         // flow types....of course we have a problem if we have
				         // saturation ne 1 and not Richards flow
			...
			Dm = Dm / (porosity * saturation);  // correct for multiplication
			                                    // with element porosities &
			                                    // saturations -> Pore diffusion
			                                    // coefficient
			//			cout << " CalcDiffusionCoefficientCP: De: " <<
			// Dm*porosity*saturation << " saturation " << saturation << "
			// porosity " << endl;
