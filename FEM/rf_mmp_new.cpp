	...
	while (!new_keyword)
	{
			...
			switch (storage_model)
			{
				...
				case 7:                             // RW/WW
					...
					double val_l =
					    ...
					    (1. -
					     storage_model_values[0] * storage_model_values[1]) /
			...
			switch (permeability_porosity_model)
			{
				...
				case 8:  // AB: 2010, Updating permeability due to change in
					...
					if (this->permeability_tensor_type == 2)
						std::cout
						    << "Warning in MMPRead: permeability_model 8 is "
						       "not implemented for permeability_tensor_type 2"
						    << "\n";
					break;
				default:
					std::cout << "Error in MMPRead: no valid permeability model"
		...
		if (line_string.find("$INTERPHASE_FRICTION") != std::string::npos)
		{
			...
			if (PhaseHeatedByFriction.compare("SOLID") != 0 &&
				...
				std::cout << "Error in CMediumProperties::Read: "
				             "$INTERPHASE_FRICTION must be either SOLID or "
				             "FLUID or NONE";
			in.clear();
			continue;
		}
	...
	if (FLOW)  // WW
	{
		...
		else
		{
			    ...
			    Fem_Ele_Std->cpl_pcs)
			{
				...
				dens_arg[1] = Fem_Ele_Std->interpolate(Fem_Ele_Std->NodalVal1) +
				              273.15;              // Temperature
                                                   ...
                                                   int tr_phase)  // SB + BG
{
	...
	// obtain a Daq-dependent alpha_t
...
double CMediumProperties::Porosity(long number, double theta)
{
	...
	switch (porosity_model)
	{
		...
		case 13:
		{
			...
			// the flow process
			// so you have to get the index of porosity above, if porosity model
			// = 13
...
void CMediumProperties::SetDistributedELEProperties(string file_name)
{
	...
	{
		line_string = GetLineFromFile1(&mmp_property_file);
		if (line_string.find("STOP") != string::npos) return;
                                                ...
                                                CFiniteElementStd* assem)
{
	...
	assem->PcsType==EnumProcessType::S);
	   ElementValue* gp_ele = ele_gp_value[index];
	   //Velocity
	   double vel[3] = {};
	...
	theta*assem->interpolate(assem->NodalVal1);
		...
		case 11:
			if (m_pcs->getProcessType() == FiniteElement::LIQUID_FLOW)
				...
				std::cout << "Wrong process type for STORAGE model 11 (only "
				             "intended for LIQUID_FLOW)." << std::endl;
			break;
		default:
		...
		case 2:
		{
			...
			else
			{
				std::cout << "***Error: Effective heat transfer model is not "
				             "supported. " << effective_heat_transfer_model
				          << '\n';
			}
