...
int CECLIPSEData::GetVariableIndex(std::string Variablename)
{
	...
	for (unsigned int i = 0; i < this->Variables.size(); i++)
		if (this->Variables[i].compare(Variablename) == 0) index_Variable = i;
...
void CECLIPSEData::ReadEclipseGrid(std::string Filename)
{
	...
	for (long i = 0; i < TextFile->NumberOfRows - 1; i++)
	{
		...
		if (tempstring.compare("COORDS  ") == 0)
		{
			...
			// should be set to 2 0 instead of 1 1! The run is terminated now!"
			//<< "\n";
			//	//system("Pause");
			//	exit(0);
			//}
			...
			if (m_eclipseblock->active == 1)
			{
				...
				this->elements += 1;
				if (m_eclipseblock->active == 1) this->activeCells += 1;
	...
	if (Radialmodell == true)
	{
		for (long i = 0; i < this->elements; i++)
			     ...
			     j++)
			{
				...
				// cout << "Element: " << i << " Point: " << j << " alpha: " <<
				// this->eclgrid[i]->y_coordinates[j] << " radius: " <<
		...
		if ((TextFile->NumberOfRows - 2) == this->elements)
			for (long i = 0; i < TextFile->NumberOfRows - 2; i++)
			{
				...
				if (Element == i)
				{
					...
					// this->eclgrid[i]->z_barycentre << "\n";
					// cout << epsilon << " " << abs(X -
					// this->eclgrid[i]->x_barycentre) << " " << abs(Y -
					// this->eclgrid[i]->y_barycentre) << " " << abs(Z -
					// this->eclgrid[i]->z_barycentre) << "\n";
					// if ((abs(atof(this->SplittedString[1].data()) -
					...
					if ((abs(X - this->eclgrid[i]->x_barycentre) < epsilon) &&
						...
						for (int j = 0; j < 6; j++)
							...
							// atoi(this->SplittedString[j + 4].data());
...
/*-------------------------------------------------------------------------
   ...
   Modification: 02/2011 KB Change from ReadSurfaceDensity to
...
int CECLIPSEData::ReadDataFromInputFile(std::string Filename)
{
	...
	for (long l = 0; l < TextFile->NumberOfRows; l++)
	{
		...
		if (tempstring.compare("INCLUDE") == 0)
		{
			...
			{
				zeilen = zeilen + 1;
			} while (TextFile->Data[l + zeilen] == "");  // look for possible
			                                             // empty rows between
			                                             // the Keyword and the
			                                             // items
		...
		if (this->existWells == true)
			if (tempstring_l.compare("WCONINJE") == 0)
			{
				...
				{
					zeilen = zeilen + 1;
				} while (TextFile->Data[l + zeilen] != "");  // look for
				                                             // possible empty
				                                             // rows between the
				                                             // Keyword and the
				                                             // items
				     ...
				     j++)
				{
					if (this->actual_time == 0)
					{
						...
						in >> dummy_rate;
						in >> dummy_zeile;
						if (!in.fail()) rest_zeile += dummy_zeile;
		...
		while (!in.eof())
		{
			    ...
			    string::npos)  // subkeyword found
			{
				while (tempstring.find("#") != 0)
				{
					...
					in.getline(Line, MAX_ZEILEN);
					tempstring = Line;
					if (tempstring.find("#") != string::npos) break;
...
bool CECLIPSEData::ReadBoundaryData(int index_boundary, vector<string> Data)
{
	...
	for (unsigned long i = 0; i < Data.size(); i++)
	{
		...
		else
		{
			...
			if (i == 0)
				if (SplittedString.size() != 3) return false;
...
void CECLIPSEData::ReadEclipseData(std::string Filename, long Timestep)
{
					...
					else
					{
						...
						if (tempNumber == this->elements)
						{
							...
							for (long l = j + 1; l < j + temprows + 1; l++)
							{
								if (l < (j + temprows))
								{
									std::string Dataline[4];  // Definition
									                          // einen strings
									                          // mit 4 Arrays,
									                          // Arrays immer in
									                          // eckigen
									...
									{
										rowindex = rowindex + 1;  // zaehlt die
										// gesplitteten Zeilen in
										// der Zeile, in Variante
										// KB4 entspricht das 4
	...
	if (this->E100 == true)
	{
		...
		if (int(this->Phases.size()) == 3)
		{
			...
			cout << "Three phases are only valid if a water saturation exists "
			        "only at the boundaries of the model domain!!";
			// system("Pause");
			// exit(0);
	...
	//" + temp.str();
	//	temp.str(""); temp.clear(); temp << elem->y_barycentre; tempstring += ";
	//" + temp.str();
	//	temp.str(""); temp.clear(); temp << elem->z_barycentre; tempstring += ";
	//" + temp.str();
...
/*-------------------------------------------------------------------------
   ...
   dissolved CO2 concentration in water
   Return: nothing
   Programming: 01/2011 BG
   Modification:
   -------------------------------------------------------------------------*/
void CECLIPSEData::CalculateRSfromMassFraction_E300()
{
	...
	if ((index_density_liquid < 0))
	{
		cout << "Not all variables are existing in the eclipse output files "
		        "that are necessary to write data back to e300!"
		     << "\n";
		// system("Pause");
		exit(0);
	}
	...
	for (long i = 0; i < this->elements; i++)
	{
		...
		// the liquid phase
		// RS [m�_CO2 / m�_liquid] = (V_liquid * rho_liquid * xw_CO2_liquid) /
		// (rho_CO2_surface * V_liquid) = rho_liquid [kg_liq / m�_liq] *
		// xw_CO2_liquid [kg_CO2 / kg_liq] / rho_CO2_surface [kg_CO2 / m�_CO2]
		...
		if (TextFile->Data[i].substr(0, Keyword.length()) == Keyword)
		{
			...
			else
			{
				cout << "Replacing a Section in the Eclipse input file is not "
				        "possible because the section length doesn't fit!"
				     << "\n";
				success = false;
				return success;
			}
                                   ...
                                   std::string Keyword_well)
{
	...
	for (long i = 0; i < TextFile->NumberOfRows; i++)
		if (TextFile->Data[i].substr(0, Keyword_well.length()) == Keyword_well)
		{
			...
			{
				zeilen = zeilen + 1;
			} while (TextFile->Data[i + zeilen] != "");  // look for possible
			                                             // empty rows between
			                                             // the Keyword and the
			                                             // items
			jj = 0;
...
bool CECLIPSEData::CompareElementsGeosysEclipse()
{
	...
	for (unsigned long i = 0; i < m_msh->ele_vector.size(); i++)
	{
		...
		// ECL " << this->eclgrid[i]->x_coordinates[j] << " " <<
		// this->eclgrid[i]->y_coordinates[j]  << " " <<
		...
		if (ElementIsEqual == false) return 0;
...
bool CECLIPSEData::CreateFaces(void)
{
	...
	// order of nodes for each element in geosys: 0..x1,y1,zoben;
	// 1..x2,y1,zoben; 2..x2,y2,zoben; 3..x1,y2,zoben;
	...
	for (unsigned long i = 0; i < m_msh->ele_vector.size(); i++)
	{
		...
		// automatically to the positve i-direction
		// radial elements: Right face of the Element (i-direction) (Nodes:
		// 6,5,2,1) -> with this order of the nodes the normal vector shows
		// automatically to the positve i-direction
		...
		// automatically to the positve j-direction
		// radial elements: Upper face of the Element (j-direction) (Nodes:
		// 6,2,7,3) -> with this order of the nodes the normal vector shows
		// automatically to the positve j-direction
		m_face = new CFaces(int(this->Phases.size()));
		m_face->index = long(faces.size());
		m_face->model_axis = "J+";
		...
		// automatically to the positve k-direction
		// radial elements: Bottom face of the Element (k-direction) (Nodes:
		// 6,5,7,4) -> with this order of the nodes the normal vector shows
		// automatically to the positve k-direction
		m_face = new CFaces(int(this->Phases.size()));
		m_face->index = long(faces.size());
		m_face->model_axis = "K+";
		...
		// the grid (i-direction) (Nodes: 0,4,3,7) -> with this order of the
		// nodes the normal vector shows automatically to the positve
		// i-direction
		// radial elements: Left face of the Element at the left (x) site of the
		// grid (i-direction) (Nodes: 7,4,3,0) -> with this order of the nodes
		...
		if (this->eclgrid[i]->NeighbourElement[0] == -1)
		{
			...
			element_indices.clear();
			element_indices.push_back(i);
			if (this->eclgrid[i]->NeighbourElement[0] > -1) return false;
			m_face->SetElements(element_indices);
			// add the face to the vector
			faces.push_back(m_face);
			for (unsigned long j = 0; j < vec_face_nodes.size(); j++)
		...
		// j-direction
		// radial elements: Lower (y) face of the Element at the lower (y) site
		// of the grid (j-direction) (Nodes: 5,1,4,0) -> with this order of the
		// nodes the normal vector shows automatically to the positve
		// j-direction
		// if (this->eclgrid[i]->row == 1){
		if (this->eclgrid[i]->NeighbourElement[2] == -1)
		{
			...
			element_indices.clear();
			element_indices.push_back(i);
			if (this->eclgrid[i]->NeighbourElement[2] > -1) return false;
			m_face->SetElements(element_indices);
			// add the face to the vector
			faces.push_back(m_face);
			for (unsigned long j = 0; j < vec_face_nodes.size(); j++)
		...
		// nodes the normal vector shows automatically to the positve
		// k-direction
		// radial elements: Top (z) face of the Element at the top (z) site of
		// the grid (k-direction) (Nodes: 2,1,3,0) -> with this order of the
		// nodes the normal vector shows automatically to the positve
		// k-direction
		// if (this->eclgrid[i]->layer == 1 /* SB->BG :changed - please check
		// "if (this->eclgrid[i]->layer == this->layers"*/){
		if (this->eclgrid[i]->NeighbourElement[4] == -1)
		{
			...
			element_indices.clear();
			element_indices.push_back(i);
			if (this->eclgrid[i]->NeighbourElement[4] > -1) return false;
			m_face->SetElements(element_indices);
			// add the face to the vector
			faces.push_back(m_face);
			for (unsigned long j = 0; j < vec_face_nodes.size(); j++)
...
bool CECLIPSEData::GetFlowForFaces(int phase_index)
{
	...
	for (long i = 0; i < long(this->faces.size()); i++)
	{
			    ...
			    1)
			{
				...
				if (m_face->model_axis == this->BC[bc_index]->boundary_position)
				{
					...
					if (m_face->phases[phase_index]->q_norm == 0)
					{
						...
						// Eclipse
						// cout << " BC Flow through face " << m_face->index <<
						// " :  " << this->BC[bc_index]->value[0] << "\n";
						vz = 1.0;
						// if(this->BC[bc_index]->value[0] < 0){ // outflow out
						// of model area
						// The values in I+, J+ and K+ direction have to be
						// counted with the opposit direction
						// because the flow from BC is given positive if fluid
						// flows from the BC into the model
		...
		// if(m_face->connected_elements.size() > 1) cout << ", " <<
		// m_face->connected_elements[1];
		// cout << ":           " << m_face->q_norm << ":   " << m_face->q[0] <<
		// ", " << m_face->q[1] << ", " << m_face->q[2] << "\n";
	}
...
bool CECLIPSEData::GetVelForFaces(void)
{
	...
	for (unsigned long i = 0; i < this->faces.size(); i++)
	{
		...
		if ((number_elements < 1) || (number_elements > 2)) return false;
		...
		if (m_face->model_axis == "I+")
		{
			...
			if (variable_index < 0) return false;
		...
		// m_face->q_norm/m_face->face_area/0.5 /m_face->v_norm ;
		// cout << "\n";
	}
	return true;
}
...
bool CECLIPSEData::MakeNodeVector(void)
{
	...
	//					  // Go through all three coordinates x, y, z and check, if
	//face
	// is perpendicular to axis
	//					  for (int k = 0; k < 3; k++){
	//						  if(k == 0) val =
	// fabs(PointProduction(normal_vec_face,coord_v_x));
	//						  if(k == 1) val =
	// fabs(PointProduction(normal_vec_face,coord_v_y));
	//						  if(k == 2) val =
	// fabs(PointProduction(normal_vec_face,coord_v_z));
	//						  if(val > MKleinsteZahl){ // face not perpendicular
	//to
	// ccordinate axis k
	...
	//<<
	// m_face->index << ", " << m_face->model_axis << "  , " << m_face->q_norm
	// <<
	//": ";
	...
	//"
	//<< k << " at node " << i << " with non-zero contributions! " << "\n";
	//					 // }
	//					}
	//				}
	//				// TEst Output
	...
	//v_z_ECL;
	// v_ECL; v_x_Geos; v_y_Geos; v_z_Geos; v_Geos");
	//
	...
	// temp.str();
	//					temp.str(""); temp.clear(); temp << m_NodeData->x; tempstring
	//+=
	//"; " +temp.str();
	//					temp.str(""); temp.clear(); temp << m_NodeData->y; tempstring
	//+=
	//"; " + temp.str();
	//					temp.str(""); temp.clear(); temp << m_NodeData->z; tempstring
	//+=
	//"; " + temp.str();
	//					for(int k=0;k< 3;k++)  {
	...
	///+ pow(m_NodeData->Flow[1],2) + pow(m_NodeData->Flow[2], 2)); tempstring
	///+= "; " + temp.str();
	//
	...
	//+
	// pow(v_geosys[1], 2) + pow(v_geosys[2], 2)); tempstring += "; " +
	// temp.str();
	//					//write string
	//					vec_string.push_back(tempstring);
	//				}
	...
	// temp.str();
	//				std::string aus_file = path + "\\CheckVelocityAtNodes_" +
	// tempstring + ".csv";
	//				ofstream aus;
	//				aus.open(aus_file.data(),ios::out);
                                                   ...
                                                   int phase_index)
{
	...
	/* Go through GeoSys mesh, and get each element (=Eclipse block). For each
	   ...
	   node, look at all faces connected to the node.
	   From each face, get the flow accross the face, and make a weighted sum in
	   the nodes.
	   Faces are not considered, if the are perpendicular to the flow direction,
	   as they then would always contribute zero
	   Faces can be choosen with two options:
	   - All faces at one node, then also information of phase velocities from
	   outside the element considered is
	   taken into account.
	...
	{          // Get the connected node
		nod_ind = nod_index[i];
		m_node = m_msh->nod_vector[nod_ind];  // m_node = Knoten, der gerade
		                                      // behandelt wird--> siehe
		                                      // Koordinaten
		...
		for (unsigned int j = 0; j < m_node->connected_faces.size(); j++)
		{
			...
			if (choose)
			{
				...
				// m_face->phases[phase_index]->q[mm] << ", ";
				// cout << "\n";
                                                    ...
                                                    int phase_index)
{
	...
	for (unsigned long i = 0; i < m_msh->nod_vector.size(); i++)
	{
		     ...
		     j++)
		{
			...
			volume = m_block->volume / 8.0;  // ToDo volume weighting doesn't
			                                 // work if element is not
			                                 // simple!!!!!!!!! BG
			weight = (1.0 / volume);
			...
			if (variable_index_vapor_mass_fraction > -1)
			{
				vap_comp_mass_frac =
				    this->Data[m_block
				                   ->index][variable_index_vapor_mass_fraction];
		...
		// density
		if (sum_weights_density > 0) density = density / sum_weights_density;
	...
	if (this->Phases.size() > 1)
		if (phase_index > 0)
		{
			...
			vec_string.push_back(
			    ...
			    "Saturation_Geosys_phase1; P-Eclipse_phase1; P-Eclipse_phase2; "
			    "Saturation_Eclipse_phase1; q_x1; q_y1; q_z1; q_x2; q_y2; "
			    "q_z2");
			for (unsigned long i = 0; i < this->NodeData.size(); i++)
			{
				m_NodeData = this->NodeData[i];
	...
	// = temp.str();
	// TextFile->Write_Text(path_new + "\\CheckNodeValues_" + tempstring +
	// ".csv", vec_string);
	//} // End Test Output
...
void CECLIPSEData::WriteDataToGeoSys(CRFProcess* m_pcs, std::string path)
{
	...
	// CB_merge_0513
	REACTINT* m_rei = NULL;
	if (REACTINT_vec.size() > 0) m_rei = REACTINT_vec[0];
	...
	if (int(this->Phases.size()) == 1)
	{
		index_pressure1 =
		    m_pcs->GetNodeValueIndex("PRESSURE1") + 1;  //+1... new time level
		index_water_density =
		    m_pcs->GetNodeValueIndex("DENSITY1");  // DENSITY1 is not a variable
		                                           // for some flow process,
		                                           // e.g. LIQUID_FLOW
		// WTP_CB: new way to get the total number of nodes in the mesh since
		// nod_val_vector is not suitable anymore
	...
	else if (int(this->Phases.size()) > 1)
	{
		...
		for (long i = 0; i < nnodes; i++)
		{
			...
			// == 599.84998) && (this->NodeData[i]->z == -2875))
			//	cout << "Punkt 44" << "\n";
			// calculating capillary pressure (difference of the two phase
			// pressures)
			// cout << " Node " << i << " (X,Y,Z): (" << this->NodeData[i]->x <<
			// ", " << this->NodeData[i]->y << ", "<< this->NodeData[i]->z << ")
			// ";
			// for (long k = 0; k < 3; k++){
		...
		if (this->GetVariableIndex("RS") >= 0)
		{
			...
			if (this->ProcessIndex_CO2inLiquid == -1)
			{
				cout << "In the model exists dissolved gas but there is no "
				        "dissolved C in water defined. Please ad the mass "
				        "transport for immobile C(4) or CO2_w!"
				     << "\n";
			...
			// since nod_val_vector is not suitable anymore
			...
			for (long i = 0; i < nnodes; i++)
			{
				...
				// [g/mol] * 1e-3 [kg/g])
				// value = this->NodeData[i]->Gas_dissolved *
				...
				if (indexConcentration > 0)
					pcs_vector[this->ProcessIndex_CO2inLiquid]->SetNodeValue(
					    i, indexConcentration - 1, value);  // 2012.07.02
		...
		if (this->GetVariableIndex("YFW2") >= 0)
		{
					    ...
					    this->dissolved_co2_ingas_pcs_name_ECL)
					{
						...
						transportphase =
						    cp_vec[pcs_vector[this->ProcessIndex_CO2inGas]
						               ->pcs_component_number]->transport_phase;
						break;
					}
				}
			}
			if (this->ProcessIndex_CO2inGas == -1)
			{
				cout << "In the model exists dissolved gas but there is no C "
				        "in the gas phase defined. Please ad the corresponding "
				        "mass transport process for CO2 in gas ! "
				     << "\n";
				// system("Pause");
				exit(0);
			}
			...
			for (long i = 0; i < nnodes; i++)
			{
				...
				// indexConcentration_gas-1, value_gas);
				// cout << "Node: " << i << " Druck: " << m_pcs->GetNodeValue(i,
				// index_pressure2) << " RS: " <<
				// this->NodeData[i]->Gas_dissolved << " Dichte: " <<
				// this->SurfaceCO2Density << " C(4): " << value << "\n";
	...
	if (1 < 0)  // SB // WTP: hu?
		    ...
		    (m_pcs->Tim->step_current == 1000))
		{
			...
			// tempstring = temp.str();
			// TextFile->Write_Text(path_new + "\\ElementValues_Geosys_" +
                                         ...
                                         std::string folder)
{
	     ...
	     this->UseSaveEclipseDataFiles == true))
	{
		...
		if (this->ecl_well.size() != 0)
		{
			...
			// this->ecl_well[ii]->phase + "   " + this->ecl_well[ii]->open_flag
			//+ "   " + this->ecl_well[ii]->control_mode + "   " +
			// this->WellRates[ii] + "   " + " / ";
			//	vecData.push_back(outline);
			//	}
	...
	if (Timestep == 1)
	{
		...
		if (int(this->Phases.size()) == 0)
		{
			...
			if (this->E100 == true)
			{
				...
				// felxible...espacially for E300
				// e.g. the Variables should be read from the .DATA (under
				// RPTRST) and written in "Variables"
				...
				if (int(this->Phases.size()) == 2)
				{
					...
					else
					{
						    ...
						    (this->Phases[1] == "GAS"))
						{
							...
							this->Variables.push_back("RS");  // gas saturation
							                                  // in oil in the
							                                  // black oil mode
							this->Variables.push_back("GAS_DEN");  // gas
							                                       // density in
							                                       // the black
							                                       // oil mode
						}
			...
			else
			{
				...
				this->Variables.push_back("MLSC1");  // total molar density for
				                                     // components(moles per
				                                     // reservoir volume)
				this->Variables.push_back("MLSC2");  // total molar density for
				                                     // components(moles per
				                                     // reservoir volume)
				this->Variables.push_back("MLSC3");  // total molar density for
				                                     // components(moles per
				                                     // reservoir volume)
	...
	else
	{
		...
		// possible to define new porosities ore permeabilties
		// every time at least the number of the time step which is used for
		// restart has to be changed
		...
		Keyword = "RESTART";
		// tempstring = "
		// d:\\Projekte\\MoPa\Programme\\Eclipse\\BGfirstmodel\\";
...
int CECLIPSEData::WriteDataBackToEclipse(CRFProcess* m_pcs, std::string folder)
{
	...
	{
		cout << "Not all variables are existing in the eclipse output files "
		        "that are necessary to write data back to e100!"
		     << "\n";
		return 0;
	}
	...
	for (long i = 0; i < nnodes; i++)
	{
		...
		// this->SurfaceCO2Density)  - this->NodeData[i]->CO2inLiquid;
		// RS = (pcs_vector[this->ProcessIndex_CO2inLiquid]->GetNodeValue(i,
		// indexConcentration) * (Molweight_CO2 * 1e-3) /
	...
	for (long i = 0; i < nele; i++)  // SB redo wtp
	{
		...
		for (long j = 0; j < long(vec_element_nodes.Size()); j++)
		{
			...
			// "\n";
			// weight = float(1.0 / vec_element_nodes.Size()); //arithmetic
			// average
			delta_gas_dis =
			    this->NodeData[vec_element_nodes[j]->GetIndex()]->deltaDIC;
		...
		if (delta_gas_dissolved < 0)
			    ...
			    (this->Data[i][variable_index_Gas_dissolved] - epsilon))
			{
				cout << "The amount of CO2 removed from water is larger than "
				        "the available amount of CO2! The value is corrected!"
				     << "\n";
				for (long j = 0; j < long(vec_element_nodes.Size()); j++)
					cout << "     Node: " << vec_element_nodes[j]->GetIndex()
					     << " RS: "
		...
		if (delta_gas_dissolved != 0)
		{
			...
			else
			{
				...
				if (this->E100 == false)
				{
					...
					// "\n";
					// T_CO2 [mol / L PV] = c_CO2inLiquid [molCO2 / m�liquid] *
					// s_CO2 [m�liquid / m�PV] * 0.001 [m�PV / L PV]
					deltaT_CO2 =
					    deltaT_CO2 *
		...
		{  // decrease in Sgas
			    ...
			    (Data[i][variable_index_Gas_saturation] - epsilon))
			{
				cout << "The volume of CO2 gas phase (Sat) removed is larger "
				        "than the available volume of CO2 gas! The value is "
				        "corrected!"
				     << "\n";
				for (long j = 0; j < long(vec_element_nodes.Size()); j++)
					cout << "     Node: " << vec_element_nodes[j]->GetIndex()
		...
		{  // decrease in Pressure2
			    ...
			    (Data[i][variable_index_Gas_pressure] - epsilon))
			{
				cout << "The pressure reduction of CO2 gas phase is larger "
				        "than the old pressure of CO2 gas! The value is "
				        "corrected!"
				     << "\n";
				for (long j = 0; j < long(vec_element_nodes.Size()); j++)
					cout << "     Node: " << vec_element_nodes[j]->GetIndex()
		     ...
		     i = i + j_max)
		{
			...
			for (int j = 0; j < j_max; j++)
			{
				...
				if (eclipse_ele_active_flag[i])
				{
					...
					ele_temp = 0.0;
					weight = float(
					    1.0 / vec_element_nodes.Size());  // getting the correct
					                                      // weight for each
					                                      // nodal value
					...
					// depending on the temperature unit used in Geosys it has
					// to be converted to °C for Ecl
					if (ele_temp < 273.15) ele_temp += 273.15;
...
int CECLIPSEData::RunEclipse(long Timestep, CRFProcess* m_pcs)
{
	...
	if ((Timestep > 1) && (this->GetVariableIndex("RS") >= 0))
	{
		if (this->UsePrecalculatedFiles == true)
			cout << "Attention! If you run a simulation with precalculated "
			        "Files you have no interaction between Multiphase Flow and "
			        "Reactions thus you might get wrong Results for dissolved "
			        "CO2 in liquid!"
			     << "\n";
		...
		if (this->Radialmodell == true)
		{
			...
			for (int ii = 0;
				...
				if (this->faces[ii]->model_axis.find("I") == 0)
				{
					...
					if (nvec[0] == 1.0) this->Radial_I = true;
				}
			if (Radial_I != true)
				cout << "\n"
	...
	if (this->E100 == false) this->CalculateRSfromMassFraction_E300();
	...
	if (UseSaveEclipseDataFiles == true) SaveEclipseDataFile(Timestep, m_pcs);
