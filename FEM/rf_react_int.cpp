	...
	while (!new_keyword)
	{
		...
		{  // subkeyword found
			...
			if (dummystring.compare("RECORD") == 0)
				...
				cout << " Warning in REACTINT::Read - No valid keyword for "
				        "$RESIDUAL (for equilibrium reaction input value)."
				     << "\n";
			in.clear();
		}
...
REACTINT* REACTINT::GetREACTINT(void)
{
	REACTINT* rei = NULL;
	/* Tests */
	if (REACTINT_vec.capacity() > 0) rei = REACTINT_vec[0];
	return rei;
}
...
void REACTINT::InitREACTINT(void)
{
	...
	if (icOutput) OUTData(0.0, aktueller_zeitschritt, true);
	...
	if (this->readNodePoro) ReadRestartNodePoro(nnodes);
					...
					{                                     // species found
						if (cp_vec[i]->iupac_formula[0])  // a formula has been
							...
							id = formula2index(cp_vec[i]->iupac_formula);
							  ...
							  // this could result in wrong concentration of
							  // basis species!!!
	...
	if (pcs_rename_init_flag == true)
	{
		...
		{
			is_idx = 1;
			if (pcs_rename0_idx_init[i] == -1) is_idx = 0;
			for (jx = 0; jx < (int)pcs_rename1_init[i].size(); jx++)
				if (pcs_rename1_idx_init[i][jx] == -1) is_idx = 0;
			if (is_idx == 0)
			{
	...
	if (pcs_rename_pre_flag == true)
	{
		...
		{
			is_idx = 1;
			if (pcs_rename0_idx_pre[i] == -1) is_idx = 0;
			for (jx = 0; jx < (int)pcs_rename1_pre[i].size(); jx++)
				if (pcs_rename1_idx_pre[i][jx] == -1) is_idx = 0;
			if (is_idx == 0)
			{
				cout << " Warning!!!, line " << i + 1
				     << " can not be found in PCS list ! "
	...
	if (pcs_rename_post_flag == true)
	{
		...
		{
			is_idx = 1;
			if (pcs_rename0_idx_post[i] == -1) is_idx = 0;
			for (jx = 0; jx < (int)pcs_rename1_post[i].size(); jx++)
				if (pcs_rename1_idx_post[i][jx] == -1) is_idx = 0;
			if (is_idx == 0)
			{
				cout << " Warning!!!, line " << i + 1
				     << " can not be found in PCS list ! "
...
{
	if (unitconversion) CalcWaterConc();
	if (s_water_limit && WaterSatLimit > 0) CheckForDriedOutNodes();
...
void REACTINT::ReactionPostProcessing(bool initial)
{
	...
	else
	{
		...
		if (dump_min_moles == true) DumpSolidSpeciesMoles();
		if (dump_all_pcs == true) DumpAllVariables();
		if (dump_mass_integrals == true) DumpMassIntegrals();
...
double REACTINT::GetPressure(long node)
{
	...
	if (constantpressure) return c_PP;
...
double REACTINT::GetTemperature(long node)
{
	...
	if (constanttemperature) return c_TT;
	...
	if (aktueller_zeitschritt == 1)
	{
		...
		for (i = 0; i < nnode; i++)
		{
			for (j = 0; j < long(Temp_GHP_xyz.size()); j++)
				if ((XYZ[i][0] == Temp_GHP_xyz[j][0]))  // x cordinate matches
					                                    // -> only regular mesh
					                                    // so far
...
void REACTINT::CalcWaterConc(void)
{
	...
	for (i = 0; i < nnodes; i++)
	{
		...
		for (j = 0; j < nc; j++)
		{
			...
			// and sp_var_ind
			// todo: what about double species, e.g. primary and secondary
			// species??
                                         ...
                                         bool molal)
{
	...
	{  // Total moles mol/m³liquid -> mol/m³REV
		*fl = node_porosity[index] * GetWaterSaturation(index);
		if (*fl == 0.0) *fl = node_porosity[index] * 1;
...
void REACTINT::SetInitialPorosityAndPermToElementValue()
{
	...
	if (poroupdate_flag == false) return;
...
void REACTINT::PorosityVolumetricReactionUpdate()
{
	...
	for (long n = 0; n < nnodes; n++)
	{
		...
		for (i = 0; i < nImmobileComp; i++)
		{
			...
			// change_in_conc[mol/m³solid]*molar_volume[m³min/mol]
			// the last factor depends on units of concentration, molweight and
			// mineraldensity
			if (mineral_densities_vector[i] > 0)
				mineral_volume_fraction +=
				    change_in_conc * (mineral_molecular_weights_vector[i] /
...
void REACTINT::DumpSolidSpeciesMoles(void)
{
	...
	if (dump_min_moles == false) return;
...
std::vector<int> REACTINT::formula2index(std::string formula)
{
	...
	for (i = 0; i < n; i++)
	{
		...
		{
			if (i > 0 && type_x[i - 1] != 2) number_ia.push_back(i);
	...
	for (i = 0; i < (int)bracket_ia0.size(); i++)
	{
		...
		for (j = 0; j < (int)bracket_ib0.size(); j++)
		{
			...
			{
				if (bracket_ib0[j] > bracket_ia0[i]) is_bracket = 1;
			}
			else
			{
		...
		if (is_bracket == 1) bracket_ia.push_back(bracket_ia0[i]);
	}
	for (i = 0; i < (int)bracket_ib0.size(); i++)
	{
		...
		for (j = 0; j < (int)bracket_ia0.size(); j++)
		{
			if (i == 0)
			{
				if (bracket_ia0[j] < bracket_ib0[i]) is_bracket = 1;
			}
			else
			{
		...
		if (is_bracket == 1) bracket_ib.push_back(bracket_ib0[i]);
	}
...
double GetNodePhaseVolume(long node, double theta, int phase)
{
	...
	for (el = 0; el < (int)m_nod->getConnectedElementIDs().size(); el++)
	{
		...
		{
			case 0:                                      // pore space
				poro = m_mat_mp->Porosity(elem, theta);  // CB Now provides also
				                                         // heterogeneous
				                                         // porosity, model 11
				break;
			case 1:                                // solid phase
