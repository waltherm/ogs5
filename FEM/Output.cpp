...
void COutput::init()
{
	if (getProcessType() == FiniteElement::INVALID_PROCESS)
	{
		std::cerr << "COutput::init(): could not initialize process pointer "
		             "(process type INVALID_PROCESS) and appropriate mesh"
		          << "\n";
...
void COutput::NODDomainWriteBinary_Header()
{
	...
	if (dat_type_name.compare("BINARY") != 0) return;
...
double COutput::NODWritePLYDataTEC(int number)
{
	...
	if (is_TECPLOT || is_GNUPLOT) tec_file_name += TEC_FILE_EXTENSION;
	if (is_CSV) tec_file_name += CSV_FILE_EXTENSION;
...
//
	...
	{
		if (is_CSV) tec_file << aktuelle_zeit << " ";
...
//
	...
	if (time_step_number == 0)  // WW  Old: if(time_step_number==1)
	{
		...
		if (is_TECPLOT || is_GNUPLOT) tec_file << " VARIABLES = ";
		tec_file << "\"TIME\" ";
	...
	if (m_sfc)
	{
		...
		for (size_t i = 0; i < nodes_vector.size(); i++)  // AB SB
		{
			...
			for (size_t k = 0; k < _nod_value_vector.size(); k++)
			{
				...
				if (_nod_value_vector[k].find("DELTA") == 0)  // JOD 2014-11-10
					tec_file << m_pcs->GetNodeValue(nodes_vector[i], 1) -
...
void COutput::ELEWritePLY_TECData(fstream& tec_file)
{
	...
	else
	{
		...
		if (m_pcs_flow == NULL) m_pcs_flow = PCSGet(FiniteElement::LIQUID_FLOW);
	}
	v_eidx[0] = m_pcs_flow->GetElementValueIndex("VELOCITY1_X");
	v_eidx[1] = m_pcs_flow->GetElementValueIndex("VELOCITY1_Y");
	v_eidx[2] = m_pcs_flow->GetElementValueIndex("VELOCITY1_Z");
                                 ...
                                 vector<double>& node_value_vector_adv)
{
	...
	FiniteElement::CElement* element =
	...
	for (i = 0; i < (long)elements_at_geo.size(); i++)
	{
		...
		for (j = 0; j < nfaces; j++)
		{
			...
			element->ConfigElement(face, m_pcs_flow->m_num->ele_gauss_points,
			                       true);  // 2D fem
			...
			///
			element->FaceNormalFluxIntegration(elements_at_geo[i], nodesFVal,
			                                   nodesFVal_adv, nodesFace, face,
...
void COutput::NODWritePrimaryVariableList(double time_current)
{
	...
	char number_char[3];
	string number_string = number_char;
	if (geo_name.size() == 0) geo_name = "domain";
		    ...
		    MKleinsteZahl)  // WW MKleinsteZahl
		{
			...
			switch (getGeoType())
			{
				...
				case GEOLIB::SURFACE:
					...
					m_sfc = GEOGetSFCByName(geo_name);
					if (m_sfc) m_msh->GetNODOnSFC(m_sfc, nodes_vector);
