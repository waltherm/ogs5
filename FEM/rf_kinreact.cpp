...
double MicrobeData::GetGibbsEnergy(long node)
{
	...
	for (int i = 0; i < m_kr->number_monod; i++)
	{
		    ...
		    (m_kr->monod[i]->isotopecouplenumber >= 0))
		{
			...
			Ctot = C +
			       cp_vec[MonodSpecies]->getProcess()->GetNodeValue(
			           node, m_krd->sp_varind[MonodSpecies]);
		}
		...
		if (m_kr->monod[i]->threshhold == true)
		{
			...
			MonodOrder =
			    (int)
			        m_kr->monod[i]->threshOrder;  // CB higher order Monod terms
			Monodterm *=
            ...
            const GEOLIB::GEOObjects& geo_obj, const std::string& unique_name)
{
	...
	while (!krc_file.eof())
	{
		...
		if (line_string.find("#MICROBE_PROPERTIES") != string::npos)
		{
			...
			if (m_md->Read(&krc_file)) MicrobeData_vector.push_back(m_md);
		}  // keyword found
...
void KRConfig(const GEOLIB::GEOObjects& geo_obj, const std::string& unique_name)
{
	...
	if (!KinReactData_vector.empty())
	{
		...
		for (size_t j = 0; j < length; j++)
		{
			...
			//" << m_pcs->GetNodeValueIndex(sp_name) + 1 << "\n";
		}
	...
	while (!new_keyword)
	{
		...
		{  // subkeyword found
			...
			else if ((T_model == 1) || (T_model == 2))
			{
				if (type.compare("monod") == 0)
				{
					...
					else if (T_model == 2)
					{
						cout << " Unknown model for Temperature Dependence of "
						        "Reaction " << name << " in CKinReact::Read(). "
						     << "\n";
						cout << " Aborting now!"
						     << "\n";
						exit(0);
				...
				{
					cout << " Warning in CKinReact::Read() - "
					        "Arrhenius-Equation is used for Temperature "
					        "correction of Mineral Reaction " << name << "\n";
				...
				else
				{
					cout << " Warning in CKinReact::Read() - Temperature "
					        "correction of " << type << " Reaction " << name
					     << " is not implemented. "
					     << "\n";
					cout << " Input values for $TEMPERATURE_DEPENDENCE keyword "
					        "will be ignored. "
			...
			{  // check for read in
				...
				while (!new_subkeyword)
				{
					    ...
					    (line_str1.find("$") != string::npos))
					{
						if (line_str1.find("#") != string::npos)
							new_keyword = true;
						rfd_file->seekg(index1);  // Dateipointer zurücksetzen,
						                          // sonst ist das nächste
						                          // subkeyword weg
						break;
					}
	...
	while (!new_keyword)
	{
		                   ...
		                   // diffenrent Sherwood-models
		{
			...
			in >> shidx;  // Sh_factor >> Re_expo >> Sc_expo >> Pe_expo >>
			              ...
			              // Poro_expo >> Delta_expo >> UI_expo >>
			              // NContent_ini_expo >> NContent_res_expo >> GSR_expo
                         ...
                         const std::string& unique_name)
{
	...
	while (!new_keyword)
	{
		...
		if (line_string.find("$ALLOW_REACTIONS") != string::npos) allow = true;
		if (line_string.find("$NO_REACTIONS") != string::npos) allow_not = true;
...
void CKinReactData::ExecuteKinReact(void)
{
	...
	if (ReactDeactFlag)
	{
		if (aktueller_zeitschritt > 2)
			ReactionDeactivation(nnodes);  // Check if nodes should be
		                                   // deactivated or activated for this
		                                   // time step
	...
	// at all nodes
	if (this->NumberNAPLdissolution > 0) NAPLDissolutionPreprocessing();
	// This function  prepares
	//  - Ionic streghts
	//  - activity coefficients
	// at all nodes and for all species
	if (this->NumberMineralkinetics > 0) PreprocessMinKin();
	if (this->NumberMicrobeData > 0) this->PreprocessMicrobe_drmc_(dt);
			...
			for (it = 0; it < nnodes; it++)
			{
				...
				for (tt = 0; tt < tsteps; tt++)
				{
					...
					else
					{
						...
						if (sortnodes)
							substeps[node] = nbad + nok;  // no of substeps as
						                                  // indicator for
						                                  // computation time

						if (tsteps > 1)
							usedt = usedtneu;  // use last successfull timestep
						                       // to continue integration over
						                       // substeps
					}
				}  // tt
				//
...
// Get MPI results
			...
			for (node_idx = 0; node_idx < nnodes; node_idx++)
			{
				...
				{
					if (m_rei->s_water_limit)
						if (m_rei->dried_out_nodes[node_idx]) dry = true;
				}
			...
			if (sortnodes) SortIterations(substeps, noderanks, nnodes);
			// for(node_idx = 0; node_idx < nnodes; node_idx++)
                                   ...
                                   int* nbad, int tt, int tsteps)
{
	...
	if (Number_of_blobs > 0)
	{
		...
		if (gassdiss == false)
			DensityAQ =
			    mfp_vector[0]->Density(variables);  // should be returned as
		...
		for (r = 0; r < nreactions; r++)
		{
			...
			if (m_kr->typeflag_napldissolution)
			{
				...
				DarcyVelocity =
				    m_kr->GetNodeDarcyVelocity(node);  // SP: added for Sherwood
				                                       // calculation after
				                                       // Powers et al. 1992
		...
		for (r = 0; r < Number_of_blobs; r++)
		{
			...
			if (m_kb->gas_dissolution_flag == false)
			{
				...
				switch (shidx)
				{
					...
					case 4:  // Sherwood calculation after Pfannkuch (1984)
						Sherwood =
						    m_kb->Sh_factor +
						    m_kb->Pfannkuch_constant * pow(Peclet, two_three);
						break;
			...
			else if (m_kb->Sherwood_model == true)  // modified Sh'= k * (d50)^2
				                                    // / Daq --> k =
				                                    // Sh*Daq/(d50)^2
				m_kb->Masstransfer_K[node] =
				...
				if (success == 0)
				{
					...
					/*else*/ if (success == 0)
					{
						cout << "   -Trying perturbation of concentration "
						     ...
						     << "\n";
						for (sp = 0; sp < Number_of_Components; sp++)
							m_Conc[sp + 1] = tempstore[sp];  // restore original
						                                     // double value
						                                     // precision
						for (sp = 0; sp < Number_of_Components; sp++)
	...
	for (r = 0; r < nreactions; r++)
	{
		...
		if (m_kr->typeflag_monod)
		{
			...
			if (BacteriaMass > 1.E-40)
			{
				...
				if (m_kr->grow)
					dcdt[BacteriaNumber] +=
					    BacGrowth * m_kr->EffectiveYield(node);  // CB 08/12
				                                                 // added a
				                                                 // microbial
				                                                 // yield
				                                                 // coefficient
				...
				for (i = 0; i < n; i++)
				{
					Yield = m_kr->ProductionStoch[i];  // CB 08/12 this is the
					                                   // stoichiometry
					                                   // coefficient of the
					                                   // reaction
		...
		for (i = 0; i < n; i++)
		{
			...
			if ((c[i + 1] + dcdt[i + 1] * dt < 0) && (dcdt[i + 1] * dt != 0))
			{
				...
				{  // species by species
					dcdt[j + 1] = 0;
					for (r = 0; r < nreactions; r++)
						dcdt[j + 1] += ratematrix[j][r];  // sum up
					                                      // contributions of
					                                      // all reactions
				}
			}
	...
	for (r = 0; r < nreactions; r++)
	{
		...
		if (m_kr->typeflag_monod)
		{
			...
			if (BacteriaMass > 1.E-40)
			{
				...
				// maximale Kapazität berücksichtigt werden muss
				// Muss sein, weil Ableitungen höhere Ordnung haben
				// (Bakterienmasse steckt auch in Kapazitätsgleichung)
				// This is for the case that growth of any bacteria is limited
				// by total bacteria density sumX & maxkap
				if ((maxkap > 1.E-30) &&
				    (speccap <
				...
				{  // bacteria specific capacity term defined
					...
					if (maxVelocity > 1.E-30)
					{
						...
						// nach allen anderen Substanzen
						//   Ableitung nach der Bakterienmasse (mit
						//   Berücksichtigung d. spezif. Kapazitätsterms)
						//   d2Xi / dt*dXi = BacGrowth *(specif_cap) /
						//   (Xi*(Xi+specif_cap))
						d2X_dtdS[BacteriaNumber] =
						    BacGrowth * (speccap) /
				...
				// maximale Kapazität berücksichtigt werden muss
				// without capacity term, this is identical for growth and decay
				// term; but not if yield coeff is considered (CB)
				else
				{  // maxkap = 0
					/* d2Xi / dt*dXi = BacGrowth / Xi */
					d2X_dtdS[BacteriaNumber] =
					...
					if (CMonodSpecies > 1.E-20)
					{
						    ...
						    (m_kr->monod[i]->species == m_kr->Isotope_light))
						{
							...
							if (m_kr->monod[i]->threshhold == true)
							{
						      ...
						      // MonodConcentration / CMonodSpecies /
						      // (MonodConcentration+CMonodSpecies); // no
						      // isofrac
						{
							d2X_dtdS[MonodSpecies] =
	...
	for (r = 0; r < nreactions; r++)
	{
		...
		if (m_kr->typeflag_mineralkinetics)
		{
			...
			if (Betrag > 0)
			{
				...
				// these are automaticaly computed below in loop over mechterms
				// and mechspecies (2)
				// Anyway, we calculate these partial derivatives based on dM/dt
				// = MineralGrowth.
				// StoechCoeff sign in factor2 must be changed because it is
				// defined negative
				// for species which in the IAP are forming the mineral (there,
				// they are positive, See fct. Omega)
				...
				// constant of rate law
				// d2M_dtdCmech = 0 + 0 + ... + K_mech*A* |1-Om^theta|^eta *
				// MechExpo_i/Cmech_i
				//
				// calculate these partial derivatives here without using
				// previously calculated dM/dt
				// as only the a term containing Cmech remains from sum after
				// taking derivative.
				// This partial derviatives adds up to a mineral species p.d.
				// from (1),
				// when it is a mech species at the same time; however, no
				...
				for (i = 0; i < no_mech; i++)
				{
					...
					if (BetragArg <= -1)           // strong supersaturation
						dXdt_Mech =
						    pow(dXdt_Mech, 1 / eta2);  //   --> slow down
					                                   //   precipitation rate
					                                   //   by root function
					                                   //   (default = 1)
					...
					if (BetragArg <= 0)      // supersaturation
						dXdt_Mech *= m_kr->precipfactor;  //   --> slow down
					                                      //   precipitation
					                                      //   rate by factor
					                                      //   (default = 1)
					for (j = 0; j < no_mechspec; j++)
					{
						...
						// species,
						// and it may ALSO be a mineral species from (1) at the
						// same time
						// therefore += results
				...
				// solver
				// rows i of J contain a single species' derivatives wrt all
				// other species d2Ci/dtdCj
				...
				// this MUST be zero for the mineral itself, as otherwise, the
				// p.d. is added here again
				porosity1 = m_kr->GetReferenceVolume(MineralNumber - 1, node);
				for (i = 0; i < n; i++)
				{                                      // these are the rows i
					Yield = m_kr->ProductionStoch[i];  // = 0 for Mineral
					                                   // (must), <0 for min
					                                   // species consumed, >0
					                                   // for species produced
					                                   // by precipitation
					if (fabs(Yield) > 1.E-30)
					{
						porosity2 = m_kr->GetReferenceVolume(i, node);
	...
	for (r = 0; r < nreactions; r++)
	{
		...
		if (m_kr->typeflag_napldissolution)
		{
			    ...
			    c[Sp1])
			{
				...
				//water-concentration
				// is zero in case of reduced rate in derivs
				//		dfdc[Sp2][Sp2] = 0
...
bool KNaplDissCheck(void)
{
	...
	{
		m_krd = KinReactData_vector[0];
		if (m_krd->NumberNAPLdissolution > 0) NAPLdiss = true;
...
bool KMinKinCheck(void)
{
	...
	{
		m_krd = KinReactData_vector[0];
		if (m_krd->NumberMineralkinetics > 0) MinKin = true;
		return MinKin;
	}
}
...
void CalcNewNAPLSat()
{
	...
	bool psg = false;
	if (m_pcs->getProcessType() == FiniteElement::PS_GLOBAL) psg = true;
	...
	if (psg) idx0 = m_pcs->GetNodeValueIndex("SATURATION2");  // old timelevel
	idx1 =
	...
	for (i = 0; i < nnodes; i++)
	{
		...
		if (KinBlob_vector[0]->gas_dissolution_flag == false)
		{
			...
			for (j = 0; j < nNAPLcomps; j++)
			{
				...
				mass += conc * molar_weights_vector[j];
				volume += conc / molar_densities_vector[j];  // this is required
				                                             // calculating the
				                                             // napl fluid
				                                             // density
			}
			...
			// set new SATURATION2
			// if(satu_N_new > 0.00001)  satu_N_new = 1-0.95; // 0.985 CB
			// 14.01.09 Hansen+Kueper BM
		...
		{  // gas_dissolution
			...
			if (psg)  // PS_GLOBAL
				...
				satu_N_old =
				    1.0 - m_pcs->GetNodeValue(i, idx2 + 1);  // idx2+1 as in
			                                                 // case of Eclipse
			                                                 // coupling, Sats
			                                                 // are transient.
...
void CalcNewPhasePressure()
{
	...
	{
		m_kr = KinReact_vector[i];
		if (m_kr->typeflag_napldissolution) break;
	}
	if (m_kr->typeflag_napldissolution != 1) return;
...
void CalcNAPLCompMasses()
{
	...
	bool mpf = false;
	if (m_pcs->getProcessType() == FiniteElement::MULTI_PHASE_FLOW) mpf = true;
	...
	for (j = 0; j < nelenodes; j++)
	{
		...
		coord = m_nod->getData();
		if (coord[2] > ymax) ymax = coord[2];
		if (coord[3] < ymin) ymin = coord[2];
	}
	area *= (ymax - ymin);
	...
	for (j = 0; j < nDisscomps; j++)
	{
		...
		idxC = 1 +
		       pcs_vector[l]->GetNodeValueIndex(
		           pcs_vector[l]->pcs_primary_function_name[0]);
		for (i = 0; i < nnodes; i++)
		{
...
double CalcNAPLDens(int node)
{
	...
	m_mfp = mfp_vector[m_pcs->pcs_type_number + 1];  // CB ToDo: check, if this
	                                                 // also applies for
	                                                 // MULTI_PHASE_FLOW
	// m_mfp = mfp_vector[m_pcs->pcs_type_number];
	// m_mfp->mode = 1; // CB ??
...
void CKinReactData::PreprocessMicrobe_drmc_(double steplength)
{
	...
	for (i = 0; i < NumberMicrobeData; i++)
	{
		...
		for (j = 0; j < (long)m_msh->nod_vector.size(); j++)
		{
			...
			// initialized with zero before
			// the first time step, update of S in the first time step hence is
			// Snew = Sold * exp(0)
			// this is an analytical approximtion of dS/dt =
		...
		for (j = 0; j < nnode; j++)
		{
			...
			for (i = 0; i < ncomp; i++)
			{
				...
				if (cp_vec[i]->compname.compare("pH") == 0)
					conc = pow(10, -conc * unitfactor_l);  // this is inacurate
				                                           // as it should be
				                                           // molality of H+,
				                                           // not the activity.
			...
			if (Imin > IonicStrengths[j]) Imin = IonicStrengths[j];
			if (Imax < IonicStrengths[j]) Imax = IonicStrengths[j];
		}  // nnodes
			...
			case 1: /* DH*/
				if (Imax > 0.1)
					DisplayMsgLn(
					    ...
					    "--> use of DH activity model is not recomended!");
				break;
			case 2: /* Davies*/
				if (Imin < 0.1)
					DisplayMsgLn(
					    "Warning in CKinReactData::PreprocessMinkin(): I < 0.1 "
					    "--> use of Davies activity model is not recomended!");
				break;
			case 3: /* CHEMAPP*/
				break;
			default:
			...
			{  // CB merge CAP 0311
				...
				for (i = 0; i < nnode; i++)
				{
					...
					if (cp_vec[j]->compname.compare("pH") == 0)
						conc = pow(10, -conc);  // this is inacurate as it
					                            // should be molality of H+, not
					                            // the activity.
						...
						for (k = 0; k < nnode; k++)
						{
							...
							{   // what, if no data has been calculated by
								...
								cout << "    No Chemapp-log(K) data available "
								        "for reaction " << m_kr->chemapp_name;
								cout << " at node " << k << "\n";
								if (k > 0)
									m_kr->Km[k] =
	...
	for (i = 0; i < NumberReactions; i++)
	{
		...
		if (m_kr->typeflag_mineralkinetics && m_kr->Am_constant == false)
		{
			...
			for (k = 0; k < nnode; k++)
			{
				...
				{
					case 1:
						if (conc > 0) m_kr->Am[k] *= conc;
						if (m_kr->Am[k] < 1e-6)
						{
							m_kr->Am[k] = 1e-6;  // minimum allowed surface area
							                     // to allow regrowth of a
							                     // mineral
							warning = true;
							// node = k;
						}
			...
			for (k = 0; k < nnode; k++)
			{
				...
				switch (m_kr->Am_model)
				{
					...
					case 2:  // for primary minerals, volume fraction weighted
						if (REACTINT_vec.size() > 0)
						{
							n0 = REACTINT_vec[0]->node_ini_porosity[k];  // here
							                                             // plug
							                                             // in
							// n_ini
							// at
					...
					case 3:  // for secondary minerals, only poro fraction
						...
						if (REACTINT_vec.size() > 0)
						{
							n0 = REACTINT_vec[0]->node_porosity[k];  // here
							                                         // plug in
							                                         // n_ini at
							                                         // node
							n1 = REACTINT_vec[0]->node_porosity[k];
...
double CKinReact::MinRate(double* c, int node, double dt, double Om)
{
			...
			if (ExplicitMinKinRateCoeff)
				...
				ktot =
				    -MinRateConstant(
				        c, node,
				        (1 -
				         H1));  // default negative rate: dissolution of mineral
	...
	//        dissolved phase volume
	//        maxrate = -conc / dt * poro2 / poro1 /
	...
	//      conc = DMAX(0,c[minSpeciesIdx[i]+1]);         // do not evaluate
	//      negative conc
	//      if(ProductionStoch[minSpeciesIdx[i]]<0){      // this is species
	//      consumed by precipitation, Stcoef < 0
	//        poro2	= GetReferenceVolume(minSpeciesIdx[i],node);         //
	//        dissolved phase volume
	//        maxrate = -conc / dt * poro2 / poro1 /
...
double CKinReact::MinRateConstant(double* c, int node, double disso)
{
	...
	for (i = 0; i < number_Mech; i++)
	{
		...
		{   // for precipitation, consider only base mechanism, if set in
			// inputfile
			if (mechvec[i]->no_mechSpec == 0) ktot += mechvec[i]->Mech(c, node);
		}
		else
...
/**************************************************************************
 ...
 direction
        Then it is sufficient to simulate flow and conservative transport at all
 nodes,
        but reactions only at one side of the element. Then copy to other side
 of element.
 Programing:
...
void CKinReactData::CopyConcentrations(void)
{
	...
	if (copy_concentrations == false) return;
	...
	if (copy_nodes.size() < 1)
	{
		...
		else if ((radial) || (TwoDinThreeD))
		{
			...
			coord0 = ele_nodes[0]->getData();  // Get coordinates of first four
			                                   // element nodes ("upper" part of
			                                   // element)
				...
				if (err_flag == false)
				{
					cout << "\n"
					     << " Copy Cons - Please switch off one model side for "
					        "reaction calculation ! - Option switched off "
					     << "\n";
					radial = TwoDinThreeD = copy_concentrations = false;
				}
			}
