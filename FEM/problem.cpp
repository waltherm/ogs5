      ...
      msize(0)
{
	...
	if (!validMatID)
	{
		std::cout << "Error: not enough MMP data. please check MMP and "
		             "material IDs in a mesh." << std::endl;
	...
	if (transport_processes.size() > 0)  // 12.12.2008. WW
	{
		...
		if (REACTINT_vec.size() > 0)
		{
			if (REACTINT_vec[0]->unitconversion)
			{
				...
				flow_pcs = PCSGetFlow();
				if (flow_pcs->type == 1212)  // in case of mutlltiphase flow,
					                         // sat water must be calculated
					                         // here, required by pgc interface
		...
		if (KinReactData_vector.size() > 0)
		{
			...
			if (MicrobeData_vector.size() > 0) MicrobeConfig();
		}
	}
//----------------------------------------------------------------------
// REACTIONS
...
// SB4501        rc->ExecuteReactions();
	...
	if (REACTINT_vec.size() > 0) REACTINT_vec[0]->ReactionPostProcessing(true);
	//----------------------------------------------------------------------
	// DDC
...
//
#if defined(USE_MPI)
		MPI_Bcast(&dt, 1, MPI_DOUBLE, 0, comm_DDC);  // all processes use the
                                                     // same time stepping
                                                     // (JT->WW. Must they
                                                     // always?)
#endif
...
bool Problem::CouplingLoop()
{
	...
	for (i = 0; i < (int)pcs_vector.size(); i++)
	{
		...
		if (pcs_vector[i]->bc_transient_index.size() != 0) transient_bc = true;
	}
	if (transient_bc) pcs_vector[0]->WriteBC();
...
MPI_Allreduce(&has_constrained_bc_i, &has_constrained_bc, 1, MPI_C_BOOL,
...
*/
		...
		if (max_outer_error <= 1.0 &&
		    outer_index + 1 >= cpl_overall_min_iterations)  // JT: error is
			                                                // relative to the
			                                                // tolerance.
			break;
...
void Problem::PostCouplingLoop()
{
	...
	if (total_processes[12])
	{
		...
		{
			if (msp_vector[l]->GetBoolExcavated()) doPostExcav = true;
		}
	    ...
	    prqvec)
	{
		...
		if (KinReactData_vector.size() > 0)
		{
			if (KinReactData_vector[0]->NumberMineralkinetics > 0)
				KinReactData_vector[0]->PostprocessMinKin();  // Set Mineral
			                                                  // surface areas
			                                                  // for next time
			                                                  // step based on
			                                                  // deltaC
...
inline double Problem::LiquidFlow()
{
	          ...
	          //         standard: use GeoSys
	{
		...
		if (m_pcs->tim_type == TimType::STEADY) m_pcs->selected = false;
	}
...
inline double Problem::MassTrasport()
{
	...
	if (REACTINT_vec.size() > 0) REACTINT_vec[0]->ReactionPreProcessing();
...
inline double Problem::RandomWalker()
{
	...
	if (m_pcs && m_pcs->selected)
	{
		...
		if (!m_msh) m_msh = FEMGet("FLUID_MOMENTUM");
