...
public:
	...
	std::string
	    simulator_well_path;  // path to well schedule ( *.well), with extension
	                          // SB redo wtp
	std::string dissolved_co2_pcs_name;  // Keyword DISSOLVED_CO2_PCS_NAME; Name
	                                     // of MASS_TRANSPORT Process which is
	                                     // used to store total dissolved CO2
	                                     // from ECLIPSE
...
// WW void CheckBCGroup(); //OK
	...
	/// Initialize the unknows of equations with existing solutions within a
	/// index range.
...
extern double PCSGetELEValue(long index, double* gp, double theta,
...
extern int ModelsAddNodeValInfoStructure(char*, char*, int, int, int, int,
                                         double);
...
extern int ModelsAddElementValInfoStructure(char*, char*, int, int, int, int,
                                            double);
