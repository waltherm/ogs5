...
ProcessType convertProcessType(const std::string& pcs_type_string)
{
	...
	if (pcs_type_string.compare("TNEQ") == 0) return TNEQ;
	if (pcs_type_string.compare("TES") == 0) return TES;
	// else
...
PrimaryVariable convertPrimaryVariable(const std::string& pcs_pv_string)
{
	...
	if (pcs_pv_string.compare("TEMPERATURE2") == 0) return TEMPERATURE2;
	...
	if (pcs_pv_string.compare("DISPLACEMENT_N") == 0) return DISPLACEMENT_N;
	...
	if (pcs_pv_string.compare("CARBON1") == 0) return CARBON1;
	if (pcs_pv_string.compare("WATER1") == 0) return WATER1;
	if (pcs_pv_string.compare("METHANE1") == 0) return METHANE1;
	if (pcs_pv_string.compare("NITROGEN1") == 0) return NITROGEN1;
	// else
	//{
...
std::string convertPrimaryVariableToString(PrimaryVariable pcs_pv)
{
	...
	if (pcs_pv == TEMPERATURE1) return "TEMPERATURE1";
	if (pcs_pv == TEMPERATURE2) return "TEMPERATURE2";
	if (pcs_pv == CARBON1) return "CARBON1";
	if (pcs_pv == WATER1) return "WATER1";
	if (pcs_pv == METHANE1) return "METHANE1";
	if (pcs_pv == NITROGEN1) return "NITROGEN1";
	...
	if (pcs_pv == DISPLACEMENT_N) return "DISPLACEMENT_N";
...
DistributionType convertDisType(const std::string& dis_type_string)
{
	...
	if (dis_type_string.compare("RECHARGE_DIRECT") == 0) return RECHARGE_DIRECT;
	if (dis_type_string.compare("DOMAIN") == 0) return NODESCONSTANT;
	if (dis_type_string.compare("CLIMATE") == 0) return CLIMATE;
...
std::string convertDisTypeToString(DistributionType dis_type)
{
	...
	if (dis_type == RECHARGE_DIRECT) return "RECHARGE_DIRECT";
	if (dis_type == NODESCONSTANT) return "DOMAIN";
	if (dis_type == CLIMATE) return "CLIMATE";
	if (dis_type == RECHARGE)  // MW
		return "RECHARGE";
	if (dis_type == FUNCTION) return "FUNCTION";  // 24.08.2011. WW
	if (dis_type == TRANSFER_SURROUNDING) return "TRANSFER_SURROUNDING";
...
{
	if (friction_string.compare("SOLID") == 0) return SOLID;
	if (friction_string.compare("FLUID") == 0) return FLUID;
	if (friction_string.compare("NONE") == 0) return NONE;
...
{
	if (friction_phase == SOLID) return "SOLID";
	if (friction_phase == FLUID) return "FLUID";
	if (friction_phase == NONE) return "NONE";
...
{
	if (reactive_string.compare("INERT") == 0) return INERT;
	if (reactive_string.compare("SINUSOIDAL") == 0) return SINUSOIDAL;
	if (reactive_string.compare("CaOH2") == 0) return CaOH2;
	if (reactive_string.compare("Mn3O4") == 0) return Mn3O4;
	if (reactive_string.compare("Z13XBF") == 0) return Z13XBF;
...
{
	if (reactive_system == INERT) return "INERT";
	if (reactive_system == SINUSOIDAL) return "SINUSOIDAL";
	if (reactive_system == CaOH2) return "CaOH2";
	if (reactive_system == Mn3O4) return "Mn3O4";
	if (reactive_system == Z13XBF) return "Z13XBF";
...
{
	if (str.compare("STEADY") == 0) return TimType::STEADY;
	if (str.compare("TRANSIENT") == 0) return TimType::TRANSIENT;
	if (str.compare("PURERWPT") == 0) return TimType::PURERWPT;
	return TimType::INVALID_TIM_TYPE;
}

std::string convertTimTypeToString(TimType::type type)
{
	if (type == TimType::STEADY) return "STEADY";
	if (type == TimType::TRANSIENT) return "TRANSIENT";
	if (type == TimType::PURERWPT) return "PURERWPT";
...
{
	if (str.compare("VELOCITY") == 0) return ConstrainedVariable::VELOCITY;
