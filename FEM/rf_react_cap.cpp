	...
	while (!new_keyword)
	{
		...
		{  // subkeyword found
			...
			while (1)
			{
				...
				if (!ncomp_x.find("END"))
				{
					if (ik > 0) this->Kin_Reactions.push_back(this->React);
					in.clear();
					break;
				}
		...
		{  // subkeyword found
			...
			while (1)
			{
				...
				if (!ncomp_x.find("END"))
				{
					if (ikh > 0) this->Kin_HKF_Reactions.push_back(this->React);
					in.clear();
					break;
				}
				else if (!ncomp_x.find("REACTION"))
...
void REACT_CAP::ExecuteReactionsChemApp(int f, int nodeflag)
{
	...
	if (f == 0)
	{
		...
		if (this->Kin_HKF_species.size() > 0) this->LoadHKFparam();
	...
	else
	{
		...
		if (m_rei)
		{
			if (m_rei->s_water_limit)
				for (l = 0; l < (long)m_msh->nod_vector.size(); l++)
					if (m_rei->dried_out_nodes[l]) rateflag[l] = 0;
		}
...
void REACT_CAP::InitChemApp(void)
{
	...
	// strcmp(m_pcs->pcs_primary_function_name[0], "Eh")==0){
...
void REACT_CAP::LoadMassPCS(void)
{
	...
	{
		is_idx = 1;
		if (idx0[i] == -1) is_idx = 0;
		for (jx = 0; jx < (int)pcs_rename1[i].size(); jx++)
			if (pcs_idx1[i][jx] == -1) is_idx = 0;
		if (is_idx == 0)
		{
	...
	{
		is_idx_pre = 1;
		if (idx0_pre[i] == -1) is_idx_pre = 0;
		for (jx = 0; jx < (int)pcs_rename1_pre[i].size(); jx++)
			if (pcs_idx1_pre[i][jx] == -1) is_idx_pre = 0;
		if (is_idx_pre == 0)
		{
			cout << " Warning!!!, line " << i + 1
			     << " can not be found in PCS name list ! "
		...
		if (rateflag[ii] != 0)
		{
			...
			if (m_rei)
			{
				if (m_rei->unitconversion)
				{
					...
					// m_rei->GetWaterSaturation(ii);
					// if(unitfactor_l==0.0) unitfactor_l =
					...
					for (i = 0; i < (size_t)ns; i++)
					{
						...
						{  // liquid phase
							    ...
							    strcmp(species_name[i], "water_liquid") == 0)
							{
								...
								// preprocessing,
								// and after kinreact anyway, so use the node
								// value
								// species_value[i] =  m_rei->water_conc[ii] *
								// unitfactor_l ; // set the total amount of
								// water
			...
			while (!is_equilibrium)
			{
				...
				{  // ChemApp eq calc recovering
					...
					{  // no relative activity
						...
						{
							if (ii ==
							    1 /*|| ii==1 || ii==8 || ii==12 || ii==16 || ii==20 */)
								CAP_tqce(const_cast<char*>(" "),
								         0,
								         0,
								         vals,
								         &noerr);  // this is faster, using
							                       // previuos result as start
							                       // for iteration
							else
								CAP_tqce(const_cast<char*>(" "),
								         0,
							...
							else
							{
								     ...
								     i++)
								{
									...
									{  // DL====
										...
										else if (this->species_relative_activity_state
										                 [i] == 2 ||
										         this->species_relative_activity_state
										                 [i] == 3)
										{  // dormant case
				...
				else
				{
					...
					for (i = 0; i < (size_t)ns; i++)
					{
						if (species_value[i] > -50)
						{
							...
							if (species_dormant[i] == 0)
								species_value[i] = value;  // get eq value from
							                               // chemapp, set to
							                               // input vector
				...
				if (m_rei)
					if (m_rei->VLE_conditions.size() > 0)
					{
						...
						for (i = 0; i < m_rei->VLE_conditions.size(); i++)
						{
							if (!iv_eq[i]) is_equilibrium = false;
						}
						iter_eq++;
						if (iter_eq >= 20)
						{
			...
			if (m_rei)
			{
				if (m_rei->VLE_pressure.size() > 0)
				{
					for (i = 0; i < m_rei->VLE_pressure.size(); i++)
					{
						if (m_rei->VLE_pressure[i].vp_value > 0) is_VLE = true;
					}
				}
			}
			...
			if (!noerr)
			{
				...
				if (MinKinReact && ff != 1 && nodeflag < 0)
				{
					...
					for (i = 0; i < species_value.size(); i++)
						species_value_d.push_back(species_value[i]);
					this->KinParamUpdata(
					    ii, noerr, species_value_d);  // in case of  no error, a
					                                  // filled vector will be
					                                  // stored
					this->KinParamUpdataHKF(
					    ii, TT, PP, noerr);  // CB include noerr
					//-----------------------------------
				...
				if (pcs_redox > -1)
				{
					...
					if (redox_stoi[1] != 0)
						value1 =
						    redox_stoi[0] +
						    8.31451 * TT / 96485.3 * value1 / redox_stoi[1];
					else
						value1 = redox_stoi[0];
	...
	{  // value was calculated
		...
		{                               // loop over species
			...
			else
			{
				...
				else if (value_ac /*spvc[i]*/ > 0)  // otherwise divide by
					                                // molality to obtain gamma,
					                                // if species conc > 0
...
vector<double> REACT_CAP::KineticReact(vector<double> species_value0)
{
	...
	// for(i=0;i<(int)species_value0.size();i++){
	//	cout << setw(32) << species_name[i] << setw(16) << species_value0[i]
	///*<< setw(10) << species_value1[i] */<< " dcdt " << dcdt0[i] << "\n";
	//}
...
/***********************************************************
      Function: REACT_CAP::ODE
                                             DL   10/10
***********************************************************/
vector<double> REACT_CAP::ODE(vector<double> c0,
                              vector<double> h,
                              int iter_max,
                              double t_step)
{
	...
	while (j < iter_max)
	{
		...
		if (j > 1)
			for (i = 0; i < (int)c0.size(); i++)
				if (dcdt0[i] * dcdt1[i] <= 0) n++;

		for (i = 0; i < (int)c0.size(); i++)
		{
			if (h[i] > 0)
			{
				if (dcdt0[i] > 0) k = 1 - pow(10, h[i]);
				if (dcdt0[i] < 0) k = 1 - pow(10, -h[i]);
				if (dcdt0[i] == 0) k = 0.0;
				if (dcdt0[i] != 0 && c0[i] != 0) t1 = abs(c0[i] * k / dcdt0[i]);
