      ...
      ele_val_name_vector(std::vector<std::string>())
{
	...
	// SB redo wtp
	dissolved_co2_pcs_name = "";  // SB, CB 10/2011 Name of MASS_TRANSPORT
	                              // process used to store dissolved total CO2
	                              // in water
...
CRFProcess::~CRFProcess(void)
{
	...
	PetscPrintf(PETSC_COMM_WORLD,
	            "\t\n>>Total Wall clock time in the assembly for %s (with "
	            "PETSC):%f s\n",
	            FiniteElement::convertProcessTypeToString(
	                this->getProcessType()).c_str(),
	            cpu_time_assembly);
	...
	// HS, 11.2011
	if (m_conversion_rate) delete m_conversion_rate;
...
{
	if (hasAnyProcessDeactivatedSubdomains) CheckMarkedElement();  // 01.2014 WW
...
// eqs_new = EQS_Vector[2*k];
	...
	//
	if (reload >= 2 && type != 4 &&
	    type / 10 != 4)  // Modified at 03.08.2010. WW
	{
		// PCH
...
void initializeConstrainedProcesses(std::vector<CRFProcess*>& pcs_vector)
{
	...
	for (std::size_t i = 0; i < pcs_vector.size(); i++)
	{
		if (!(pcs_vector[i]->hasConstrainedBC())) continue;
...
void CRFProcess::Config(void)
{
	...
	if (hasAnyProcessDeactivatedSubdomains) CheckMarkedElement();  // WW
...
//#define DECOVALEX
		...
		pcs_secondary_function_timelevel[pcs_number_of_secondary_nvals] = 1;
		pcs_number_of_secondary_nvals++;
		// for calibration (unconfined groundwater flow with variable density)
		// 5.3.07 JOD
...
//		exit(1);
	...
	if (m_num->nls_method_name.find("PICARD") != string::npos)
	{
		...
		if (myrank == 0) cout << "      PCS error: " << pcs_error << "\n";
		...
		else
		{
			for (int ii = 0; ii < pcs_number_of_primary_nvals; ii++)
			{
				...
				for (j = 0; j < g_nnodes; j++)
				{
					...
					SetNodeValue(j, nidx1, (1.0 - nl_theta) * val_n +
					                           nl_theta * eqs_x[k]);
					eqs_x[k] = val_n;  // Used for time stepping. 03.04.2009. WW
#else
					k = m_msh->Eqs2Global_NodeIndex[j];
...
// unsigned int counter;	//void warning
		...
		if (!onDeactiveBoundary) continue;
//
#if defined(USE_PETSC)  // || defined(other parallel libs)//03~04.3012. WW
                                    ...
                                    CNodeValue const& st_node)
{
	...
	for (std::size_t i = 0; i < st.getNumberOfConstrainedSTs(); i++)
	{
		    ...
		    local_constrained.constrainedPrimVar == FiniteElement::TEMPERATURE)
		{
			...
			for (std::size_t k = 0; k < pcs->GetPrimaryVNumber(); k++)
			{
				        ...
				        local_constrained.constrainedPrimVar))
				{
					...
					// expected
					// however, getPressureAsHead is part of CBoundaryCondition
					// (m_bc in IncorporateBoundaryConditions)
					...
					{
						if (local_value >=
						    local_constrained.constrainedValue)  // check if
							                                     // calculated
							                                     // value (eg of
							                                     // other
							                                     // process)
		...
		if (constrained_bool) return_value = true;
	}
			...
			for (std::size_t k = 0; k < pcs->GetPrimaryVNumber(); k++)
			{
				        ...
				        local_constrained.constrainedPrimVar))
				{
					                               ...
					                               // values
					{
						...
						double local_value(
						    0.1 *
						        pcs->GetNodeValue(bc_node.geo_node_number,
						                          2 * k + 1) +
						    0.9 *
						        pcs->GetNodeValue(bc_node.geo_node_number,
						                          2 * k));
		...
		if (node_id >= sorted_node)  // found range
		{
			...
			else
			{
				           ...
				           // node)
				{
					...
					{
						new_GWL_st_node =
						    this->m_msh->sorted_nodes[j - 1];  // for p<0,
						                                       // return node id
						                                       // two nodes
						                                       // below gwl
						break;
...
void CRFProcess::IncorporateSourceTerms(const int rank)
{
	...
	{
		// for constrainedST
		if (!(st_vector[i]->isConstrainedST())) continue;

		for (std::size_t j(0); j < st_vector[i]->getNumberOfConstrainedSTs();
		     j++)
		{
			if (!(st_vector[i]->isCompleteConstrainST(j))) continue;
		    ...
		    FiniteElement::CLIMATE)
		{
			...
			if (Tim->step_current < 1000)
			{
				...
				{
					filename << 0;
					if (Tim->step_current < 10) filename << 0;
				}
			}
...
void CRFProcess::CalcSecondaryVariables(bool initial)
{
	...
	switch (getProcessType())
	{
		...
		case FiniteElement::TES:
			...
			break;
		case FiniteElement::LIQUID_FLOW:
			if (aktueller_zeitschritt > 0) CalcSecondaryVariablesLiquidFlow();
			break;
		case FiniteElement::GROUNDWATER_FLOW:
			break;
		case FiniteElement::TWO_PHASE_FLOW:
					...
					{  // Check for stagnation
						percent_difference =
						    100.0 * ((last_error - nonlinear_iteration_error) /
						             last_error);
						if (iter_nlin > 0 &&
						    percent_difference < 1.0)  // less than 1%
							                           // difference (or an
							                           // error increase) from
							                           // previous error
							num_fail++;
...
void CRFProcess::CalcSecondaryVariablesLiquidFlow()
{
	...
	CRFProcess* m_pcs = NULL;
	if ((m_pcs = PCSGet("HEAT_TRANSPORT"))) heattransport = true;
                                    ...
                                    int timelevel)
{
	...
	switch (pcsT)
	{
		...
		case 1313:  // Two_phase_Flow
			...
			if (m_pcs)
			{
				idx = m_pcs->GetNodeValueIndex(var_name);  // +timelevel; CB
				                                           // SATURATION1 in
				                                           // PS_GLOBAL is only
				                                           // for old time level
				cplpcs = m_pcs;
...
void CRFProcess::AssembleParabolicEquationRHSVector(CNode* m_nod)
{
	...
	for (size_t i = 0; i < 3; i++)
		if (v_eidx[i] < 0)
		{
			cout << v_eidx[i] << i
			     << " Warning in "
			        "CRFProcess::AssembleParabolicEquationRHSVector - no "
			        "PCS-VEL data"
			     << "\n";
			return;
		}
		...
		switch (m_ele->GetElementType())
		{
			...
			default:
				cout << "Warning in "
				        ...
				        "implemented for this element type"
				     << "\n";
				break;
		}  // switch
	}
...
void CRFProcess::CalculateFluidDensitiesAndViscositiesAtNodes(CRFProcess* m_pcs)
{
	...
	Molweight_NaCl =
	    58.443;  // [g/mol]				//ToDo: provide constants once in the
	             // whole project
	...
	for (long i = 0; i < (long)m_msh->nod_vector.size(); i++)
	{
		...
		for (size_t j = 0; j < m_node->getConnectedElementIDs().size(); j++)
		{
			...
			node_volume =
			    node_volume +
			    m_ele->GetVolume() / m_ele->GetNodesNumber(false) * porosity;
		}
		...
		if (saturation_liquid > 0)
		{
			...
			for (int j = 0; j < iter_max; j++)
			{
				...
				// fraction of solvent = mol per kg solvent
				// Molality of CO2 in mol per kg pure water (mol/m³ * m³/kg) =
				// mol/kg
			...
			// 1e-3 - c_NaClinLiquid * Molweight_NaCl * 1e-3) / (Molweight_H2O *
			// 1e-3);	//[mol]
		...
		if (saturation_gas > 0)
		{
			...
			for (int j = 0; j < iter_max; j++)
			{
				...
				// compare density with density estimation
				if (Density_gas < 0) break;
				if (fabs(Density_gas - temp_density) < epsilon) break;
				if (Density_gas < temp_density) b = temp_density;
				if (Density_gas > temp_density) a = temp_density;
			}
...
void CRFProcess::Phase_Transition_CO2(CRFProcess* m_pcs, int Step)
{
	...
	// for initial conditions variable index is the first index, for other
	// timesteps variable index is the second index
	if (m_pcs->Tim->step_current == 1) TimeStepVariableIndex = 0;
	...
	for (long i = 0; i < (long)m_msh->nod_vector.size(); i++)
	{
		...
		// c_CO2inLiquid_old = pcs_vector[indexProcess]->GetNodeValue(i,
		// variable_index);		//[mol/m³]
		// if (i == 230)
		//	cout << i << "\n";
		// liquid.H2O = (Density_liquid_old * liquid.volume - c_CO2inLiquid_old
		// * liquid.volume  * Molweight_CO2 * 1e-3 - liquid.NaCl *
		...
		// (Molweight_H2O * 1e-3);	//[mol]
		// else {
		// indexProcess = MassTransportID[0];
		// variable_index =
		// pcs_vector[indexProcess]->GetNodeValueIndex(pcs_vector[indexProcess]->pcs_primary_function_name[0])
		// + 1; // +1: new timelevel
		// c_H2OinLiquid = pcs_vector[indexProcess]->GetNodeValue(i,
		// variable_index);		//[mol/m³]
		// liquid.H2O = c_H2OinLiquid * liquid.volume;										//[mol] =
		// mol/m³
		// * m³
		//}
		...
		if (m_pcs->Tim->step_current == 14) cout << i << "\n";
