                                            ...
                                            bool& valid)
{
	...
	while (!new_keyword)
	{
		...
		if (line_string.find("$CONSTRAINED") != std::string::npos)
		{
			...
			in >> tempst >> tempst2;  // VELOCITY and DIRECTION
			                          // (positive/negative scalar product
			                          // between velocity vector and surface
			                          // normal); or PROCESS_TYPE and associated
			                          // PRIMARY_VARIABLE
			if (tempst == "VELOCITY")
			{
				      ...
				      temp.constrainedDirection == ConstrainedType::NEGATIVE))
				{
					std::cout << "No valid constrainedDirection for "
					          << convertConstrainedVariableToString(
					                 temp.constrainedVariable) << "(" << tempst2
					          << ")" << std::endl;
					_isConstrainedBC = false;
				}

				in >> tempst;
				if (tempst == "STABLE") temp._isConstrainedVelStable = true;

				if (getGeoType() != GEOLIB::SURFACE)
					std::cout << "\n Warning! Make sure, that a velocity "
					             "constrained BC is a SURFACE!" << std::endl;
			}
			else
			{
				...
				if (tempst == "SEEPAGE")
				{
					if (temp.constrainedDirection == ConstrainedType::SMALLER)
						...
						std::cout << "Seepage not used as constrained "
						             "direction is not set to SMALLER.\n "
						             "Please check .bc file." << std::endl;
				}
			}
			if (_isConstrainedBC) this->_constrainedBC.push_back(temp);
			in.clear();
		}
		//....................................................................
	}
						...
						for (size_t i(0); i < nodes_vector_size; i++)
						{
							...
							double const* const coords(
							    m_msh->nod_vector[m_node_value->geo_node_number]
							        ->getData());
							m_node_value->node_value =
							    bc->dis_linear_f->getValue(coords[0], coords[1],
							                               coords[2]);
			...
			if (bc->getGeoType() == GEOLIB::SURFACE)
			{
				...
				if (m_surface)
				{
					    ...
					    FiniteElement::LINEAR)
					{
						...
						// WW
						node_value.resize(nodes_vector_length);
						bc->SurfaceInterpolation(pcs, nodes_vector,
						                         node_value);  // WW
					}
