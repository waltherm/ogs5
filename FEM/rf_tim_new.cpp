				             ...
				             TimeControlType::STABLE_ERROR_ADAPTIVE)
				{
					       ...
					       (!tim_file->eof()))
					{
						          ...
						          isdigit(line_string[0])))
						{
							...
							{
								std::cout << "ERROR: at least two multipliers "
								             "should be provided for "
								             "SELF_ADAPTIVE time stepping"
								          << std::endl;
								exit(1);
							}
						}
...
double CTimeDiscretization::CalcTimeStep(double current_time)
{
	...
	for (int i = 0; i < (int)critical_time.size(); i++)
	{
		if (current_time < critical_time[i])
		{
			...
			tval = next + time_step_length / 1.0e3;  // JT2012. A tiny increase
			                                         // in dt is better than a
			                                         // miniscule dt on the next
			                                         // step
	...
	for (size_t n_p = 0; n_p < pcs_vector.size(); n_p++)
	{
		...
		switch (m_pcs->getProcessType())  // TF
		{
			...
			case FiniteElement::MASS_TRANSPORT:  // TF, if steady state, time
				...
				{
					// time step will be reduced in an exponential way until
					// min_time_step.
					time_step_length =
					    pow(time_adapt_coe_vector[time_adapt_coe_vector.size() -
					                              1],
					        (double)rejected_step_count) *
					    initial_step_size;

					if (time_step_length < min_time_step)
					{
						std::cout << "-> ***ERROR*** Next time step size is "
						             "less than the given minimum size. The "
						             "simulation is aborted." << std::endl;
						exit(1);
					}
				}
				break;
			...
			case FiniteElement::RICHARDS_FLOW:  // TF
			{
				...
				if (time_control_type == TimeControlType::SELF_ADAPTIVE)  // MW
				{
					...
					{
						std::cout << "-> ***ERROR*** Next time step size is "
						             "less than or equal to the given minimum "
...
double CTimeDiscretization::StableErrorAdaptive(void)
{
	...
	if (aktueller_zeitschritt == 0)
	{
		...
		if ((rejected_step_count < 1) && SEA_parameters_are_bad()) return 0;
...
bool CTimeDiscretization::isDynamicTimeFailureSuggested(CRFProcess* m_pcs)
{
	...
	if (time_control_type != TimeControlType::DYNAMIC_VARIABLE) return false;
	//
...
double CTimeDiscretization::CheckTime(double const c_time, const double dt0)
{
	...
	double time_forward;
	bool ontime = false;
	if ((int)time_vector.size() == 1) return dt0;
	//
	// WW please check +1
...
bool IsSynCron()
{
	...
	for (i = 0; i < (int)time_vector.size(); i++)
		if (time_vector[i]->dt_sum < DBL_MIN) count++;
