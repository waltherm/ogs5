...
void REACT::ExecutePQCString(void)
{
	...
	for (std::size_t j = 0; j < rankrankliststore[myrank].size(); j++)
	{
		...
		for (std::size_t i = 0; i < ranknodeliststore[idy].size(); i++)
		{
			...
			{
				pqc_file.seekg(0L, ios_base::beg);
				ok = WriteInputPQCString(idx, rankliststringstore[myrank][j],
				                         firstinput);
				firstinput = false;
				ii++;
			}
	...
	if (mysize_all > mysize)  // if group_IPQC exists, send the input strings to
		...
		for (std::size_t j = 1; j < rankrankliststore[myrank].size(); j++)
		{
			...
			// signal = myrank_all;
			MPI_Isend(&strlength, 1, MPI_INT, idy, myrank, MPI_COMM_WORLD,
			          &req1);
			...
			MPI_Send(message, tmp.length() + 1, MPI_CHAR, idy, myrank,
			         MPI_COMM_WORLD);
		}
#endif
	...
	if (mysize_all > mysize)  // if group_IPQC exists, receive the output
		...
		{
			MPI_Recv(&strlength, 1, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG,
			         MPI_COMM_WORLD, &status1);
			char string_RT[strlength + 1];
			MPI_Recv(string_RT, strlength + 1, MPI_CHAR, status1.MPI_SOURCE,
	...
	if (ok)
	{
		...
		MPI_Barrier(comm_DDC);
		MPI_Allreduce(Concentration_buff, Concentration, ntot * nodenumber,
...
int REACT::WriteInputPQCString(
    long index, /*ifstream *pqc_iinfile,*/ stringstream* out_buff,
    bool firstinput)
{
	char line[MAX_ZEILE];
	std::stringstream in;
	...
	if (REACTINT_vec.size() > 0)
	{
		...
		{
			m_rei->CalcUnitConversionFactors(index, &unitfactor_l,
			                                 &unitfactor_s, true);
		}
	}
	cout.flush();
			...
			*out_buff << "#GRID " << index + 1 << "\n";
			    ...
			    0)  // pH and H+ in GeoSys species, calculate pH from H+
			{
				dval1 = fabs(pcs_vector[pqc_process[n1 + 1]]->GetNodeValue(
				    index, pqc_index[n1 + 1]));  // CB 01/11 probably safer
				                                 // taking abs before
				                                 // calculating log(H+)
				// CB 19.1.2011
...
int REACT::ReadOutputIPQC(std::vector<int> ranknodelist, stringstream* ein,
                          double* m_Conc)
{
	int ok = 0;
	int ntot;
	...
	{
		m_tim = time_vector[0];
		if (m_tim->step_current == 0)  // instead, do not update values for
			                           // solid species in initial pqc
			                           // computation
			initial = true;            // dix = -1;
	}
	...
	for (std::size_t nn = 0; nn < ranknodelist.size(); nn++)
	{
		...
		{
			if (*ein >> dval)
				if (initial == false) m_Conc[position + j] = dval;
		}
...
int REACT::ReadReactionModelNew(ifstream* pqc_infile)
{
		...
		if (line_string.find("KINETICS") != string::npos)
		{
			while (line_string.find("#ende") == string::npos)
			{
				...
				if (line_string.find("# comp") != string::npos)
				{
					...
					found = 0;
					for (int i = 0; i < int(pqc_index.size()); i++)
						if (speciesname.compare(pqc_names[i]) == 0) found++;
					if (found == 0)
					{
						n_kin_phases += 1;
						pqc_names.push_back(
						    speciesname);  // store species name in vector names
						pqc_process.push_back(m_pcs->pcs_number);  // store
						                                           // process
						                                           // name in
						                                           // vector
						                                           // processes
		...
		{  // keyword found
			while (line_string.find("#ende") == string::npos)
			{
				...
				if (line_string.find("# comp") != string::npos)
				{
					...
					pqc_names.push_back(
					    speciesname);  // store species name in vector names
					pqc_process.push_back(m_pcs->pcs_number);  // store process
					                                           // name in vector
					                                           // processes
					pqc_index.push_back(idx);
				}
	...
	if ((this->gamma_Hplus > 0))
		cout << " pH found and H+ found, pH for PHREEQC input is calculated "
		        "using gamma_H+ and [H+] " << gamma_Hplus << "\n";
		...
		if (this->rateflag[index] > 0)
		{
			...
			if (m_rei)
			{
				if (m_rei->unitconversion)
				{
					...
					// m_rei->GetWaterSaturation(index));
					// if(unitfactor_s ==0) unitfactor_s = (1 -
					// m_rei->node_porosity[index]) * MOLH2OPERKG /
					// (m_rei->water_conc[index] * m_rei->node_porosity[index] *
					// 1);
				}
			}
			/* skip one line, if keyword steps larger than 1 even more lines */
