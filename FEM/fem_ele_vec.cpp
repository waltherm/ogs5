...
// Local assembly of stiffness matrix, B^T C B
			...
			for (k = 0; k < act_r; k++)
			{
				...
				for (l = 0; l < nnodes; l++)
					for (j = 0; j < ele_dim; j++)
					{
						...
						if (j == 0) dN_dx += shapefctHQ[ka] / Radius;

						f_buff = fac * dN_dx * shapefct[l];
						(*PressureC)(nnodesHQ* j + ka, l) += f_buff;
						if (PressureC_S)
							(*PressureC_S)(nnodesHQ* j + ka, l) +=
							    f_buff * fac1;
						if (PressureC_S_dp)
							(*PressureC_S_dp)(nnodesHQ* j + ka, l) +=
							    f_buff * fac2;
					}
			}
		}
		else
		{
			for (k = 0; k < act_r; k++)
			{
				...
				for (l = 0; l < nnodes; l++)
					...
					{
						f_buff =
						    fac * dshapefctHQ[nnodesHQ * j + ka] * shapefct[l];
						(*PressureC)(nnodesHQ* j + ka, l) += f_buff;
						if (PressureC_S)
							(*PressureC_S)(nnodesHQ* j + ka, l) +=
							    f_buff * fac1;
	...
	if (rho > 0.0 && GravityForce)
	{
		...
		for (k = 0; k < act_r; k++)
		{
			...
			//        (*RHS)(i+ka) += LoadFactor * rho * smat->grav_const *
			//        shapefctHQ[ka] * fkt;
		}
	}
}
...
void CFiniteElementVec::GlobalAssembly_Stiffness()
{
	...
	if (act_nodes_h != nnodesHQ) non_ghost_flag = -1;
...
void CFiniteElementVec::GlobalAssembly_RHS()
{
	...
	k = 0;
	int idx_p1_ini = 0,
	    idx_p2_ini = 0;  //, idx_sw_ini;//WX:08.2011 neglect ini h affect
	...
	if (Residual)
	{
		...
		if (excavation)
		{
			...
			for (int i = 0; i < nnodes; i++)
			{
				...
				{
					elem = pcs->m_msh
					           ->ele_vector[node->getConnectedElementIDs()[j]];
				...
				if (onExBoundary) onExBoundaryState[i] = 1;
			}
		}
		...
		switch (Flow_Type)
		{
			...
			case 1:  // Richards flow
			{
				...
				{
					val_n = h_pcs->GetNodeValue(nodes[i], idx_P1);
					S_e = (m_mmp->SaturationCapillaryPressureFunction(-val_n) -
...
// DECOVALEX
						...
						if (pcs->Neglect_H_ini == 2)
						{
							...
							S_e_ini =
							    (sw_ini - m_mmp->capillary_pressure_values[1]) /
							    (m_mmp->capillary_pressure_values[2] -
							     m_mmp->capillary_pressure_values[1]);
						}
						...
						if (smat->bishop_model > 0)
						{
							switch (smat->bishop_model)
							{
								case 1:
									bishop_coef_ini =
									    S_e_ini * smat->bishop_model_value;
									AuxNodal[i] = LoadFactor * S_e *
									              smat->bishop_model_value *
									              val_n;
									break;
								case 2:
									bishop_coef_ini =
									    pow(S_e_ini, smat->bishop_model_value);
									AuxNodal[i] =
									    LoadFactor *
					...
					if (pcs->Neglect_H_ini == 2)  // WX:08.2011
					{
						if (smat->bishop_model == 1 ||
						    smat->bishop_model == 2 || smat->bishop_model == 3)
							AuxNodal[i] -=
							    LoadFactor * bishop_coef_ini *
							    h_pcs->GetNodeValue(nodes[i], idx_p1_ini);
						else
						{
							double p0 =
				...
				{  // Multi-phase-flow: p_g-Sw*p_c
					...
					if (h_pcs->Neglect_H_ini == 2)
					{
						...
						// idx_sw_ini =
						// h_pcs->GetNodeValueIndex("SATURATION1_Ini");
					}
					...
					if (smat->bishop_model > 0)
					{
						...
						for (i = 0; i < nnodes; i++)
						{
							...
							switch (smat->bishop_model)
							{
								...
								case 2:
									...
									if (pcs->Neglect_H_ini == 2)
									{
										S_e_ini =
										    (sw_ini -
										     m_mmp->capillary_pressure_values
										         [1]) /
										    (m_mmp->capillary_pressure_values
										         [2] -
										     m_mmp->capillary_pressure_values
							...
							if (smat->bishop_model == 1 ||
							    smat->bishop_model == 2 ||
							    smat->bishop_model ==
							        3)  // pg-bishop*pc 05.2011 WX
							{
								val_n =
								    h_pcs->GetNodeValue(nodes[i], idx_P2) -
								    bishop_coef *
								        h_pcs->GetNodeValue(nodes[i], idx_P1);
								if (onExBoundaryState[i] == 1)  // WX:02.2013
									val_n = 0.;
					...
					else
					{
						...
						PressureC_S->multi(AuxNodal0, AuxNodal1,
						                   -1.0 * LoadFactor);
					}
...
// RHS->Write();
	...
	/***************************************************************************
	   ...
	   06/2004   WW   Generalize for different element types as a member of
	 class
	   12/2005   WW   Creep
	 **************************************************************************/
	void CFiniteElementVec::LocalAssembly_continuum(const int update)
	{
		...
		if (PoroModel == 4)
			// OK deporo =
			// PCSGetElementPorosityChangeRate(index)/(double)ele_dim;
			// MX  deporo =
		...
		//
		if (PModel != 3 && smat->Youngs_mode != 2)  // modified due to
		...
		for (gp = 0; gp < nGaussPoints; gp++)
		{
			...
			else
			{
				switch (PModel)
				{
...
#ifdef JFNK_H2M
						if (smat->StressIntegrationDP(gp, eleV_DM, dstress,
						                              dPhi, update) &&
						    !JFNK)
#else
					if (smat->StressIntegrationDP(gp, eleV_DM, dstress, dPhi,
							...
							smat->ConsistentTangentialDP(ConsistDep, dPhi,
							                             ele_dim);
						break;
					case 10:  // Drucker-Prager model, direct integration. 02/06
						      // WW
						if (smat->DirectStressIntegrationDP(gp, eleV_DM,
						                                    dstress, update))
						{
							*ConsistDep = *De;
					...
					case 11:  // WX: 08.2010
					{
						    ...
						    gp, De, eleV_DM, dstress, update, mm))
						{
							...
							{
								*ConsistDep = *De;
								smat->TangentialDPwithTensionCorner(ConsistDep,
								                                    mm);
								dPhi = 1.0;
							}
							break;
					...
					case 2:  // Rotational hardening model
						...
						if (smat->CalStress_and_TangentialMatrix_SYS(
						        gp, eleV_DM, De, ConsistDep, dstress, update) >
						    0)
							dPhi = 1.0;
						break;
					case 3:  // Generalized Cam-Clay model
						...
						/*
						   else if (smat->SwellingPressureType==4)  //Power.
						   07.05.2008 WW
						   {
						   double suc = interpolate(AuxNodal1);
						   double dsuc = interpolate(AuxNodal);
						   ...
						   S_Water =
						   m_mmp->SaturationCapillaryPressureFunction(suc);
						   dS = S_Water -
					...
					case 44:  // WX:12.2011 Mohr-Coloumb	bedding
						if (smat->StressIntegrationMOHR_Aniso(
						        gp, eleV_DM, dstress, update, De))
						{
							*ConsistDep = *De;
						...
						break;
						/*case 5:
						   if(smat->StressIntegrationHoekBrown(gp, eleV_DM,
						   dstress, update, De))
						   {
						   *ConsistDep = *De;
						         smat->TangentialHoekBrown(ConsistDep);
						   //also for tension
						         //ConsistDep->Write();
						         dPhi = 1.0;
						   }
			...
			if (smat->SwellingPressureType == 1)
			{
				...
				for (i = 0; i < 3; i++)
					dstress[i] -=
					    2.0 * S_Water * dS * smat->Max_SwellingPressure;
			}
	...
	void CFiniteElementVec::ExtropolateGuassStrain()
	{
		...
		if (pcs->ExcavMaterialGroup > -1)
		{
			...
			for (i = 0; i < nnodes; i++)
			{
				for (size_t jj = 0;
				     jj <
				         MeshElement->nodes[i]->getConnectedElementIDs().size();
				     jj++)
				{
					tmp_excavstate =
					    pcs->m_msh
	...
	void CFiniteElementVec::ExtropolateGuassStress()
	{
		...
		// strains:
		//---------------------------------------------------------
		avgESxx = avgESyy = avgESzz = avgESxy = avgESxz = avgESyz = avgPls =
		    0.0;
		if (this->GetExtrapoMethod() == ExtrapolationMethod::EXTRAPO_AVERAGE)
		{
			// average
	...
	void CFiniteElementVec::CheckNodesInJumpedDomain()
	{
		...
		X0[0] = 0.5 *
		        ((*eleV_DM->NodesOnPath)(0, 0) + (*eleV_DM->NodesOnPath)(0, 1));
		X0[1] = 0.5 *
		        ((*eleV_DM->NodesOnPath)(1, 0) + (*eleV_DM->NodesOnPath)(1, 1));
		X0[2] = 0.5 *
		        ((*eleV_DM->NodesOnPath)(2, 0) + (*eleV_DM->NodesOnPath)(2, 1));
	...
	**************************************************************************/
	bool CFiniteElementVec::LocalAssembly_CheckLocalization(CElem * MElement)
	{
		...
		if (ep > 0.0)  // in yield status
		{
			if (!eleV_DM->Localized)
			{
				...
				normXi =
				    sqrt(TensorMutiplication2(pr_stress, pr_stress, ele_dim));
	...
	/**************************************************************************
	   GeoSys - Funktion:
	    void CFiniteElementVec::IntersectionPoint(const int O_edge, const double
	k,
	   ...
	   06/2004   WW  Erste Version
	**************************************************************************/
	int CFiniteElementVec::IntersectionPoint(const int O_edge,
	                                         const double* NodeA, double* NodeB)
	{
		...
		for (i = 0; i < numf; i++)
		{
			...
			if (i != O_edge)
			{
				...
				else
				{
					...
					k2 = -n1 / n2;
					NodeB[0] = (NodeA[1] - xA[1] + k1 * xA[0] - k2 * NodeA[0]) /
					           (k1 - k2);
	...
	**************************************************************************/
	void CFiniteElementVec::LocalAssembly_EnhancedStrain(const int update)
	{
		...
		loc_dilatancy =
		    smat->Al * sqrt(2.0 / (1.0 - 3.0 * smat->Al * smat->Al));
		double Hd = 0.5 * smat->Hard_Loc / (1.0 - 3.0 * smat->Al * smat->Al);

		/*
		   // Reference
		   loc_dilatancy =
		   1.5*sqrt(2.0)*smat->Al*sqrt(1.0-6.0*smat->Al*smat->Al);
		...
		for (gp = 0; gp < nGaussPoints; gp++)
		{
			...
			else
			{
				...
				// Stiffness contributed by enhanced strain to the stiffness
				// matrix
				for (int i = 0; i < nnodesHQ; i++)
				{
					setTransB_Matrix(i);
	...
	/*----------------------------------------------------------------
	 ...
	 |    6   |  m        |
	   ----------------------
	   -----------------------------------------------------------------*/
	ElementValue_DM::ElementValue_DM(CElem * ele, const int NGP,
	                                 bool HM_Staggered)
	...
	ElementValue_DM::~ElementValue_DM()
	{
		...
		if (scalar_aniso_comp) delete scalar_aniso_comp;  // WX:09.2011
		if (scalar_aniso_tens) delete scalar_aniso_tens;
	...
	double CFiniteElementVec::CalcStrain_v()
	{
		for (int j(0); j < ns; j++)
		{
			...
			for (int i = 0; i < nnodesHQ; i++)
				dstrain[j] +=
				    pcs->GetNodeValue(nodes[i], Idx_Strain[j]) * shapefctHQ[i];
		}
		double val = 0;
		for (int i = 0; i < 3; i++)
