...
int REACT_PRQ::isKey(std::string pies0)
{
	...
	for (i = 0; i < 8; i++)
	{
		if (pies0 == PHREEQC_KEYWORD[i]) res = i;
	}
	return res;
}
...
void REACT_PRQ::SetInterface(void)
{
	...
	for (i = 0; i < (int)idx.size(); i++)
	{
		...
		for (j = 0; j < no_pcs; j++)
		{
			if (pcs_name[j] == PHREEQC_TEMPLATE[i][0]) ips = j;
		}
		ipcs.push_back(ips);
	}
	...
	{  // ii==0 as boundary point without reaction calc
		...
		if (m_rei)
		{
			if (m_rei->unitconversion)
			{
				...
				// m_rei->node_porosity[ii]) * MOLH2OPERKG /
				...
				if (idx_key[i] != 1 || f != -1)
				{
					for (j = 0; j < n; j++)
					{
						if (this->phrq_id_pos[i] == j)
						{
							...
							// if(cp_vec[idx]->transport_phase==0)
							// // liquid phase
							// dval /= unitfactor_l;
	...
	if (in.good())
	{
		...
		for (i = 0; i < (int)pies.size(); i++)
		{
			...
			for (j = 0; j < (int)pcs_name.size(); j++)
				if (pies[i] == pcs_name[j]) pcs_id[i] = j;
		}
