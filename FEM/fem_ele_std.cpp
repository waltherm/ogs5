...
void CFiniteElementStd::SetMemory()
{
	...
	if (pcs->Memory_Type == 0)
	{
		if (PcsType == EPT_MULTIPHASE_FLOW || PcsType == EPT_PSGLOBAL ||
			...
			Mass->LimitSize(nnodes, nnodes);  // Mass->LimitSize(nnodes); //
		                                      // unsymmetric in case of
		                                      // Upwinding
...
void CFiniteElementStd::CalCoefMassMCF()
{
	...
	if (FluidProp->cmpN > 0)
	{
		for (in = 0; in < nDF - 2; in++)
			retardation_factore[in] = 1.0 +
			                          (1.0 - poro) * SolidProp->Density(0) *
			                              FluidProp->Kd[in] * pow(poro, -1.0);
		for (in = 2; in < nDF; in++)
...
void CFiniteElementStd::CalCoefLaplace(bool Gravity, int ip)
{
	...
	switch (PcsType)
	{
		...
		case EPT_GAS_FLOW:  // Air flow
			...
			dens_arg[1] =
			    interpolate(NodalValC1) + PhysicalConstant::CelsiusZeroInKelvin;
			dens_arg[2] = Index;
...
void CFiniteElementStd::CalcAdvection()
{
	...
	for (gp = 0; gp < nGaussPoints; gp++)
	{
		...
		if (multiphase)  // 02/2007 WW
		{
			...
			dens_aug[1] =
			    interpolate(NodalVal1) + PhysicalConstant::CelsiusZeroInKelvin;
			rho_gw = 0.0;
			if (MediaProp->heat_diffusion_model == 1)
			{
				PG = interpolate(NodalValC1);
...
void CFiniteElementStd::CalcRHS_by_ThermalDiffusion()
{
	...
	for (i = 0; i < nnodes; i++)
	{
		...
		{
			(*RHS)[i] -= (*Laplace)(i, j) *
			             (NodalValC[j] + PhysicalConstant::CelsiusZeroInKelvin);
...
{
	if ((coordinate_system) % 10 != 2 && (!axisymmetry)) return;
	...
	for (in = 0; in < nDF * nnodes; in++)
		NodalVal[in] = 0.0;
	cshift = 0;
	if (nDF > 1) cshift = NodeShift[pcs->continuum];
	for (size_t k = 0; k < dim; k++)
...
void CFiniteElementStd::Assemble_Gravity_Multiphase()
{
	...
	//
	int dof_n = 1;  // 27.2.2007 WW
	if (PcsType == EPT_MULTIPHASE_FLOW) dof_n = 2;
		...
		if (this->SolidProp->getSolidReactiveSystem() != FiniteElement::INERT)
		{
			...
			{  // Fuer CaOH2 im Moment
				...
				std::cout << "Error: CMake option OGS_USE_CVODE needs to be "
				             "set to solve this process type!" << std::endl;
				exit(1);
#endif
			}
		}
...
void CFiniteElementStd::Cal_Velocity()
{
	...
	// TODO [CL] shouldn't that be equal to pcs->dof
	int dof_n = 1;
	if (PcsType == EPT_MULTIPHASE_FLOW || PcsType == EPT_PSGLOBAL) dof_n = 2;
	if (PcsType == EPT_THERMAL_NONEQUILIBRIUM) dof_n = 4;
	if (PcsType == EPT_TES) dof_n = 3;
	//
	gp_t = 0;
			...
			if (k == 2 && (!HEAD_Flag) && FluidProp->CheckGravityCalculation())
			{
				...
				if (dim == 3 && ele_dim == 2)
				{
					vel[dim - 1] += coef;  // NW local permeability tensor is
					                       // already transformed to global one
					                       // in CalCoefLaplace()
					if (PcsType == EPT_MULTIPHASE_FLOW ||
					    PcsType == EPT_PSGLOBAL)
					{
...
void CFiniteElementStd::Cal_Velocity_2()
{
	...
	if (PcsType == EPT_TWOPHASE_FLOW) SetCenterGP();  // CB 11/2007
...
void CFiniteElementStd::add2GlobalMatrixII()
{
	...
	if (pcs->GetContinnumType() == 1) dof = 1;
...
void CFiniteElementStd::CalcSatution()
{
	...
	for (i = 0; i < nnodes; i++)
	{
		...
		if (MediaProp->permeability_saturation_model[0] == 10 &&
		    eS < MediaProp->capillary_pressure_values[1])  // MW: limit to
			                                               // non-negative
			                                               // saturation for
			                                               // stability in
			                                               // unconfined gw
	...
	switch (dof_index)
	{
		case 0:
			...
			TG =
			    interpolate(NodalValC1) + PhysicalConstant::CelsiusZeroInKelvin;
			TG0 =
			    interpolate(NodalValC) + PhysicalConstant::CelsiusZeroInKelvin;
			PG2 = interpolate(NodalVal_p2);
			rhow = FluidProp->Density();
			poro = MediaProp->Porosity(Index, pcs->m_num->ls_theta);
