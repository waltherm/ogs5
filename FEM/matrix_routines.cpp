...
{
	long info
	    [2]; /* Muss immer zuerst kommen, enthaelt Dimension und Matrixtyp */
	long max_size;
...
#ifdef ERROR_CONTROL
	if (param1 < 0) MX_Exit("M1CreateMatrix", 0);
#endif
...
#ifdef ERROR_CONTROL
	if (param1 < 0) MX_Exit("M2CreateMatrix", 0);
#endif
...
#ifdef ERROR_CONTROL
	if (param1 < 0) MX_Exit("M34CreateMatrix", 0);
#endif
...
#ifdef ERROR_CONTROL
	if (w == NULL) MX_Exit("M1ResizeMatrix", 1);
	if (dimension < 0) MX_Exit("M1ResizeMatrix", 0);
#endif
...
#ifdef ERROR_CONTROL
	if (w == NULL) MX_Exit("M2ResizeMatrix", 1);
	if (dimension < 0) MX_Exit("M2ResizeMatrix", 0);
#endif
...
#ifdef ERROR_CONTROL
	if (w == NULL) MX_Exit("M34ResizeMatrix", 1);
	if (dimension < 0) MX_Exit("M34ResizeMatrix", 0);
#endif
...
#ifdef ERROR_CONTROL
	if ((i >= dim) || (j >= dim) || (i < 0) || (j < 0)) MX_Exit("M1Set", 2);
#endif
...
int M2Set(long i, long j, double aij)
{
...
#ifdef ERROR_CONTROL
	if ((i >= dim) || (j >= dim) || (i < 0) || (j < 0)) MX_Exit("M2Set", 2);
#endif
	...
	if (fabs(aij) < MKleinsteZahl) return 0; /* Abbruch bei Nullwert */
...
int M34Set(long i1, long k1, double aik)
{
...
#ifdef ERROR_CONTROL
	if ((i >= dim) || (k >= dim) || (i < 0) || (k < 0)) MX_Exit("M34Set", 2);
#endif
	...
	if (i > k)
	{                             /* Untermatrixelement */
		if (!(w->usym)) return 0; /* symmetrische Matrix, ignorieren */
		u = 1;
		i = k;
		k = i1;
	}
...
#ifdef ERROR_CONTROL
	if ((i >= dim) || (j >= dim) || (i < 0) || (j < 0)) MX_Exit("M1Inc", 2);
#endif
...
#ifdef ERROR_CONTROL
	if ((i >= dim) || (j >= dim) || (i < 0) || (j < 0)) MX_Exit("M2Inc", 2);
#endif
...
#ifdef ERROR_CONTROL
	if ((i >= dim) || (k >= dim) || (i < 0) || (k < 0)) MX_Exit("M34Inc", 2);
#endif
...
#ifdef ERROR_CONTROL
	if ((i >= dim) || (j >= dim) || (i < 0) || (j < 0)) MX_Exit("M1Mul", 2);
#endif
...
#ifdef ERROR_CONTROL
	if ((i >= dim) || (j >= dim) || (i < 0) || (j < 0)) MX_Exit("M2Mul", 2);
#endif
...
#ifdef ERROR_CONTROL
	if ((i >= dim) || (k >= dim) || (i < 0) || (k < 0)) MX_Exit("M34Mul", 2);
#endif
...
#ifdef ERROR_CONTROL
	if ((i >= dim) || (j >= dim) || (i < 0) || (j < 0)) MX_Exit("M1Get", 2);
#endif
...
double M2Get(long i, long j)
{
...
#ifdef ERROR_CONTROL
	if ((i >= dim) || (j >= dim) || (i < 0) || (j < 0)) MX_Exit("M2Get", 2);
#endif
	...
	while (k < Zeil2(i).anz)
	{                                           /* Alle Eintraege durchsuchen */
		if (j == Ind2(i, k)) return Aik2(i, k); /* gefunden! */
		k++;
	}
...
double M34Get(long i1, long k1)
{
...
#ifdef ERROR_CONTROL
	if ((i >= dim) || (k >= dim) || (i < 0) || (k < 0)) MX_Exit("M34Get", 2);
#endif

	if (i == k) return w->Diag[i]; /* Diagonalelement */
	if (i > k)
	{
		u = w->usym;
		i = k;
	...
	while (j < Sp34(k).anz && i >= Ind34(k, j))
	{
		if (i == Ind34(k, j)) return Aik34(k, j, u);
		j++;
	}
...
#ifdef ERROR_CONTROL
	if ((i >= dim1) || (j >= dim1) || (i < 0) || (j < 0)) MX_Exit("M5Get", 2);
#endif
...
#ifdef ERROR_CONTROL
	if ((vektor == NULL) || (ergebnis == NULL)) MX_Exit("M1MatVek", 3);
#endif
	for (i = 0; i < dim; i++)
	{
		ergebnis[i] = 0.0;
...
#pragma cdir nodep
	...
	for (k = 0; k < dim1; k++)
	{
		...
		if (Size > jd_ptr_max) jd_ptr_max = Size;
#ifdef SX
#pragma cdir nodep
#endif
		for (i = 0; i < Size; i++)
...
#endif
	for (k = 0; k < dim1; k++)
		if (Zeil2(k).anz > itpack_max) itpack_max = Zeil2(k).anz;
...
#ifdef ERROR_CONTROL
	if ((vektor == NULL) || (ergebnis == NULL)) MX_Exit("M2MatVek", 3);
#endif
...
#ifdef _OPENMP
#pragma omp parallel for private(j) shared(temp_ergebnis, jdiag, b, num, \
                                           col_ind)
#endif
#ifdef SX
#pragma cdir nodep
...
#ifdef ERROR_CONTROL
	if ((x == NULL) || (r == NULL)) MX_Exit("M34Mat(T)Vek", 3);
#endif
...
#ifdef ERROR_CONTROL
	if ((vektor == NULL) || (ergebnis == NULL)) MX_Exit("M1MatTVek", 3);
#endif
...
#ifdef ERROR_CONTROL
	if ((vektor == NULL) || (ergebnis == NULL)) MX_Exit("M2MatTVek", 3);
#endif
...
void MXResiduum(double* x, double* b, double* ergebnis)
{
	...
	if (b == ergebnis) MX_Exit("MXResiduum", 5);
#endif
...
void MXRandbed(long ir, double Ri, double* ReSei)
{
	...
	if (fabs(diag) < DBL_MIN) diag = MKleinsteZahl;
	...
	switch (matrix_type)
	{
		...
		case 5:  // WW/PA  08/02/2006
		{
			...
			CNode const* const nod_i(
			    mesh->nod_vector[mesh->Eqs2Global_NodeIndex[i]]);  // TODO check
			                                                       // this. i
			                                                       // could be
			                                                       // 0.
...
#ifdef ERROR_CONTROL
	if ((index >= dim) || (index < 0)) MX_Exit("MXEliminateIrrNode", 2);
	if (rechts == NULL) MX_Exit("MXEliminateIrrNode", 3);
#endif
	...
	switch (matrix_type)
	{
		default:
		{
			...
			for (j = 0l; j < dim; j++)
			{
				 ...
				 * wurde */
				if (fabs(MXGet(j, index)) > 0.) MXSet(j, index, 0.);
			} /* endfor j */
...
#ifdef ERROR_CONTROL
	if (b == NULL || x == NULL) MX_Exit("M1Vorkond", 3);
#endif
...
#ifdef ERROR_CONTROL
	if (b == NULL || x == NULL) MX_Exit("M2Vorkond", 3);
#endif
	//======================================================================
	switch (aufgabe)
	{
...
#ifdef ERROR_CONTROL
	if (b == NULL || x == NULL) MX_Exit("M34Vorkond", 3);
#endif
				...
				if (w->stat < 2) /* Matrix ist noch nicht zerlegt */
				{
					for (k = 0; k < dim; k++) /* Spalten reduzieren */
					{
						...
						for (jk = 0; jk < Sp34(k).anz; jk++)
						{
							...
							if (u) Bik34(k, jk, u) *= w->PreD[zk];
							Dkk -= Bik34(k, jk, u) * Oik;
						}
						if (fabs(Dkk) < fastNull) /*sign */
...
void MXDumpGLS(const char* name, int modus, double* rechts, double* ergebnis)
{
	...
	{                             /* Nicht-Null-Elemente zaehlen */
		for (j = 0; j < dim; j++) /* !quadratisches Problem! (Ra) */
			if (fabs(MXGet(i, j)) > MKleinsteZahl) NonZeroMatrix++;
		if (fabs(rechts[i]) > MKleinsteZahl) NonZeroRHS++;
	}
