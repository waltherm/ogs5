			...
			switch (Time_Dependent_E_nv_mode)
			{
				...
				default:
					cout << "!ERROR in msp file, no valid "
					        "TIME_DEPENDENT_YOUNG_POISSON mode!" << endl;
					break;
			}
			in_sd.clear();
		}
...
// conversion_rate::conversion_rate
{
	if (reaction_system.compare("CaOH2") == 0)
	{
		...
		reaction_enthalpy =
		    -1.12e+05 / 0.018;  // in J/kg (Molar heat of reaction divided by
		                        // molar mass of water) negative for exothermic
		                        // composition reaction
	...
	else if (reaction_system.compare("Mn3O4") == 0)
	{
		...
		reaction_enthalpy =
		    -1.376e+05 / 0.032;  // in J/kg (Molar heat of reaction divided by
		                         // molar mass of oxygen) negative for
		                         // exothermic composition reaction
...
CSolidProperties::~CSolidProperties()
{
	...
	if (TransMatrixA) delete TransMatrixA;              // WX:08.2011
	...
	if (TmpMatrix2) delete TmpMatrix2;
	if (Dep_l) delete Dep_l;
	if (dDep_l) delete dDep_l;
	if (dGds_dFds) delete dGds_dFds;              // WX:08.2011
	if (TransMicroStru) delete TransMicroStru;    // WX:11.2011
	if (MicroStruTensor) delete MicroStruTensor;  // WX:11.2011
	if (TransMicroStru_T) delete TransMicroStru_T;
	if (TransMicroStru_TInv) delete TransMicroStru_TInv;
	if (comp_para) delete comp_para;
	if (tens_para) delete tens_para;
...
double CSolidProperties::Heat_Conductivity(double refence)
{
	...
	switch (Conductivity_mode)
	{
		...
		case 30:  // Another model for bentonite. 10.2013. WW
		{
			...
			const double* k_T = data_Conductivity->getEntryArray();
			//		val = k_T[0] - (k_T[0]-k_T[1]) / (1 + exp(10.0 * (refence -
			// k_T[2])));
		...
		switch (Time_Dependent_E_nv_mode)
		{
			...
			case 2:
				cout << "WARNING: for isotropic elasticity, please use "
				        "TIME_DEPENDENT_YOUNG_POISSON mode=1" << endl;
				break;
			default:
...
void CSolidProperties::CalculateCoefficent_MOHR(
    double ep, double scalar_comp, double scalar_tens)  // WX:11.2010, 09.2011
{
	int valid = 1;
    ...
    const int Update, Matrix* Dep)
{
	...
	*ConstitutiveMatrix = (0.);
	...
	if (normdstr < MKleinsteZahl) return 0;
	// if(Size==4)
	//	devStr[4]=devStr[5]=0.;
	...
	else
	{
		...
		for (i = 0; i < Size; i++)
		{
			TryStr_buff[i] = TmpStress[i];  // all stress in local cordinate
			                                // sys.
		}
	...
	if (shearsurf > 1e-5 || tensionsurf > 1e-5)
	{
		...
		bool first_step;
		double shearsurf_k1,
		    tensionsurf_k1 /*, local_damp, tmp_pos = 1., tmp_neg=1.*/;  // Newton
		// downhill
			...
			while (1)
			{
					...
					*TmpMatrix2 = (0.);
					...
					for (i = 0; i < Size; i++)
						dAniso_dsig_tens[i] =
						    ...
						    (sig_pq_sig_pq * sig_pq_sig_pq);  // instead
					                                          // TmpStress[i]
					                                          // with TmpStrTens
				...
				else
				{
					...
					else if (tensionsurf_k1 < 0)
					{
						...
						if (fabs(lamda_neg - lamda_pos) < 1e-8) break;
			...
			while (1)
			{
				...
				else
				{
					...
					else if (shearsurf_k1 < 0)
					{
						...
						if (fabs(lamda_neg - lamda_pos) < 1e-12) break;
						// dlamda = (lamda_pos+lamda_neg)/2.;
					}
					else
					{
...
double CSolidProperties::CalAnisoPara(double* Stress, double* MicroStruTensor)
{
	...
	if (LkLk < MKleinsteZahl) return 1;
	...
	*TmpDe = (0.);
	...
	if (Plasticity_Bedding)  // WX:09.2011
	{
		if (itesteps == 2 || Update > 0)
		{
			...
			if (scalar_tens < MKleinsteZahl) scalar_tens = 1;
                                         ...
                                         double* dstrain, double temperature)
{
	...
	switch (Creep_mode)
	{
		...
		case 3:
			fac = 1.5 * dt *
			      ((*data_Creep)(0) * exp(-(*data_Creep)(2) /
			                              (8.314472 * (temperature + 273.15))) *
			           pow(norn_S, (*data_Creep)(1)) +
			       (*data_Creep)(3) * exp(-(*data_Creep)(5) /
			                              (8.314472 * (temperature + 273.15))) *
                                          ...
                                          double temperature)
{
	...
	if (norn_S <= threshold_dev_str) return;
	//
	for (i = 0; i < ns; i++)
	...
	if (threshold_dev_str >= 0) norm_str = min(norm_str, max_etr);
