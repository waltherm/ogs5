...
CRFProcessDeformation::~CRFProcessDeformation()
{
	...
	if (type == 41 &&
	    (idata_type == write_all_binary || idata_type == read_write))
	{
...
void CRFProcessDeformation::Initialization()
{
	...
	if (Neglect_H_ini == 2) InitialNodeValueHpcs();
	if (Neglect_H_ini == 1) CalIniTotalStress();
...
void CRFProcessDeformation::InitialNodeValueHpcs()
{
	...
	if (fem_dm->h_pcs == NULL) return;

	tmp_h_pcs = fem_dm->h_pcs;
	int h_pcs_type = tmp_h_pcs->type;
	int idv_p_ini, idv_p1_ini, idv_p2_ini, idv_p_1, idv_p1_1,
	    idv_p2_1;  // idv_sw_ini, idv_sw_1;
	size_t i;
	if (h_pcs_type == 1 || h_pcs_type == 41)  // Liquide Flow
	{
		...
		for (i = 0; i < m_msh->GetNodesNumber(false); i++)
			tmp_h_pcs->SetNodeValue(i, idv_p_ini,
			                        tmp_h_pcs->GetNodeValue(i, idv_p_1));
	}
	...
	else if (h_pcs_type == 1212 || h_pcs_type == 42)  // Multi Phase Flwo
	{
		...
		for (i = 0; i < m_msh->GetNodesNumber(false); i++)
		{
			tmp_h_pcs->SetNodeValue(i, idv_p1_ini,
			                        tmp_h_pcs->GetNodeValue(i, idv_p1_1));
			tmp_h_pcs->SetNodeValue(i, idv_p2_ini,
			                        tmp_h_pcs->GetNodeValue(i, idv_p2_1));
			// tmp_h_pcs->SetNodeValue(i,idv_sw_ini,tmp_h_pcs->GetNodeValue(i,idv_sw_1));
		}
	}
	return;
	...
	for (std::size_t i = 0; i < m_msh->ele_vector.size(); i++)
	{
		...
		if (elem->GetMark())  // Marked for use, it is necessary here
		{
			...
			for (int gp = 0; gp < NGS; gp++)
			{
				for (int j = 0; j < 3; j++)
				{
					if (SMat->biot_const < 0 && pw < 0)  // if biot is negative
						                                 // value, negative pw
						                                 // is not considered
...
void CRFProcessDeformation::InitialMBuffer()
{
	...
	if (m_num->nls_method != 2) ARRAY = new double[bufferSize];
...
double CRFProcessDeformation::Execute(int loop_process_number)
{
	...
	if (ExcavMaterialGroup > -1) CheckExcavedElement();  // WX:07.2011
...
/// Linear solver
			...
			if (!elasticity) Norm = eqs_new->GetVecNormRHS();
...
// TODO
			...
			if (!elasticity)
			{
				...
				if (ite_steps == 1 && this->first_coupling_iteration)
				{
					InitialNorm = Norm;
					InitialNormU0 = NormU;
					if (counter == 1) InitialNormU = NormU;
				}
				...
				if (Norm < Tolerance_global_Newton && Error > Norm)
					Error = Norm;
				//           if(Norm<TolNorm)  Error =
				//           0.01*Tolerance_global_Newton;
...
//
				...
				if (InitialNorm < 10 * Tolerance_global_Newton) break;
	...
	if (m_num->nls_method != 2) RecoverSolution();
//
#ifdef NEW_EQS  // WW
#if defined(USE_MPI)
	dom->eqsH->Clean();
		...
		for (i = 0; i < pcs_number_of_primary_nvals; i++)
		{
			...
			if (u_type == 0)
			{
				...
				for (j = 0; j < number_of_nodes; j++)
				{
					...
					{
						if (i > problem_dimension_dm) continue;
...
void CRFProcessDeformation::PostExcavation()
{
	...
	{
		if (msp_vector[l]->excavated) deact_dom.push_back(l);
	}
	// if(ExcavMaterialGroup>=0&&PCS_ExcavState<0)	//WX:01.2010.update pcs
	// excav
	// state
	if (ExcavMaterialGroup >= 0)
	{
		for (size_t l = 0; l < m_msh->ele_vector.size(); l++)
		{
			    ...
			    static_cast<int>(m_msh->ele_vector[l]->GetPatchIndex()))
			{
				...
				{
					if (m_msh->ele_vector[l]->GetExcavState() == 1)
						m_msh->ele_vector[l]->SetExcavState(0);  // 1=now,
					                                             // 0=past
	    ...
	    now_Excav)  // WX:01.2011 modified for coupled excavation
	{
		...
		bool done;
		ElementValue_DM* eleV_DM = NULL;
		if ((fem_dm->Flow_Type == 0 || fem_dm->Flow_Type == 2 ||
		     fem_dm->Flow_Type == 1) &&
		    Neglect_H_ini == 2)
		{
			int tmp_type, idx_p1_ini, idx_p2_ini, idx_p1_1,
			    idx_p2_1;  //, idx_sw_1;//idx_sw_ini,
			CRFProcess* h_pcs = NULL;
			h_pcs = fem_dm->h_pcs;
			tmp_type = h_pcs->type;
			...
			for (size_t i = 0; i < m_msh->GetNodesNumber(false); i++)
			{
				...
				h_pcs->SetNodeValue(i, idx_p1_ini,
				                    h_pcs->GetNodeValue(i, idx_p1_1));
				if (tmp_type == 1212)
				{
					// h_pcs->SetNodeValue(i, idx_p1_ini, h_pcs->GetNodeValue(i,
					// idx_p1_1));
					h_pcs->SetNodeValue(i, idx_p2_ini,
					                    h_pcs->GetNodeValue(i, idx_p2_1));
					// h_pcs->SetNodeValue(i, idx_sw_ini, h_pcs->GetNodeValue(i,
					// idx_sw_1));
				}
			}
		}
		for (size_t l = 0; l < m_msh->ele_vector.size(); l++)
		{
			eleV_DM = ele_value_dm[l];
			//
			(*eleV_DM->Stress0) = (*eleV_DM->Stress);  // if not assembly the
			                                           // excavated eles in next
			                                           // time step
			//
			elem = m_msh->ele_vector[l];
			done = false;
...
void CRFProcessDeformation::UpdateIniStateValue()
{
	...
	if (Neglect_H_ini == 1) CalIniTotalStress();
	if ((fem_dm->Flow_Type == 0 || fem_dm->Flow_Type == 1 ||
	     fem_dm->Flow_Type == 2) &&
	    Neglect_H_ini == 2)
	{
		int tmp_type;
...
{
	if (!(idata_type == write_all_binary || idata_type == read_write)) return;

	if ((aktueller_zeitschritt % nwrite_restart) > 0 && (!last_step)) return;
	...
	for (k = 0; k < SizeSt; k++)
	{
		...
		for (j = 0; j < problem_dimension_dm; j++)
			for (i = 0; i < number_of_nodes; i++)
			{
				...
				if (!exist) st_node_value.push_back(m_node_value);
			}
	}
	//
