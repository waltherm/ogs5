...
// OK411        max_iter = NodeListLength;
	...
	for (;;)
	{
		...
		MXResiduum(x, b, r);
		k++;
		if (linear_error_type == 4) eps = cg_eps * (VEKNORM_BICGSTAB(x, n));

		if (VEKNORM_BICGSTAB(r, n) <= eps) break;

		if (k >= cg_maxiter) break;
...
int SpJOR(double* b, double* x, long n)
{
	...
	if (cg_maxiter > 0) max_iter = cg_maxiter;
// OK411    if (cg_maxiter == -1)
// OK411        max_iter = NodeListLength;
...
void ludcmp_3(double* a, long n, long* indx)
{
	...
	for (j = 0l; j < n; j++)
	{
		...
		indx[j] = imax;
		if (fabs(a[j * n + j]) < MKleinsteZahl) a[j * n + j] = MKleinsteZahl;
		if (j != n)
		{
			dum = 1.0 / a[j * n + j];
			for (i = (j + 1l); i < n; i++)
...
// OK411        max_iter = NodeListLength;
	...
	for (;;)
	{
		...
		k++;
		if (linear_error_type == 4) eps = cg_eps * VEKNORM_BICG(x, n);
		if (VEKNORM_BICG(r, n) <= eps) break;
...
// error_rel << endl;
	...
	if ((k >= max_iter) || (VEKNORM_BICGSTAB(r, n) <= eps)) goto end;
...
#pragma cdir nodep
		...
		if (fabs(tt) > DBL_MIN)
		{
			if ((log(fabs(ts)) - log(tt)) < log(DBL_MAX)) /* Alles o.k. */
				omega = ts / tt;
			else
...
#pragma cdir nodep
		...
		if (linear_error_type == 4) eps = cg_eps * (VEKNORM_BICGSTAB(x, n));
		if (VEKNORM_BICGSTAB(r2, n) <= eps) break;
...
// OK411        max_iter = NodeListLength;
	...
	for (;;)
	{
		...
		if (linear_error_type == 4) eps = cg_eps * (VEKNORM_QMRCGSTAB(x, n));
		if (VEKNORM_QMRCGSTAB(r, n) <= eps) break;
...
int SpMGMRES(double* b, double* x, long n)
{
	...
	if (cg_maxiter >= 0) max_iter = cg_maxiter;
	if (cg_maxiter == -1) max_iter = NodeListLength;
...
// OK411        max_iter = NodeListLength;
	...
	for (;;)
	{
		...
		k++;
		if (linear_error_type == 4) eps = cg_eps * VEKNORM_CG(x, n);
		if (VEKNORM_CG(r, n) <= eps) break;
		if (k >= max_iter) break;
...
#define GERMAN
	...
	for (;;)
	{
		...
		{
			if (iter == 0) nonlinear_rel_cg_eps = nonlinear_rel_cg_eps / 10.;
			...
			if (cg_eps < cg_eps_original) cg_eps = cg_eps_original;
		}
		/**************************************************************************/
		/* Fehlerabfrage */
		...
		/*    MKopierVec(bs,b,n); */
		if (nonlinear_convergence_type == 1) MKopierVec(xs, x, n);
	}
...
/*int SpNEWTON ( double *b, double *x, long n, void (*f)(), long ind ) */
{
	...
	for (;;)
	{
		...
		if (error <= nonlinear_abs_eps) break;
		if (k >= nonlinear_maxiter) break;
		if ((k % nonlinear_assemble) == 0) f(b, x, ind);
	}
