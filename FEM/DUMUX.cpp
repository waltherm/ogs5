                                   ...
                                   long Timestep)
{
	...
	for (size_t i = 0; i < this->NodeData.size(); i++)
	{
		...
		// tempstring += "; " + temp.str();
		//		temp.str(""); temp.clear(); temp << this->NodeData[i]->y;
		// tempstring += "; " + temp.str();
		//		temp.str(""); temp.clear(); temp << this->NodeData[i]->z;
		// tempstring += "; " + temp.str();
		...
		// temp.str();
		//		temp.str(""); temp.clear(); temp.precision(12); temp <<
		// this->NodeData[i]->phase_saturation[0]; tempstring += "; " +
		// temp.str();
		//		temp.str(""); temp.clear(); temp.precision(12); temp <<
		// this->NodeData[i]->phase_saturation[1]; tempstring += "; " +
		// temp.str();
		//		temp.str(""); temp.clear(); temp.precision(12); temp <<
		// this->NodeData[i]->phase_density[0]; tempstring += "; " + temp.str();
		//		temp.str(""); temp.clear(); temp.precision(12); temp <<
		// this->NodeData[i]->phase_density[1]; tempstring += "; " + temp.str();
		//		temp.str(""); temp.clear(); temp.precision(12); temp <<
		// this->NodeData[i]->CO2inLiquid; tempstring += "; " + temp.str();
		temp.str("");
		temp.clear();
		temp.precision(12);
                               ...
                               long Timestep)
{
	...
	// X_CO2inBrine X_NaClInBrine  T rho_liquid rho_gas
...
void CDUMUXData::WriteDataToGeoSys(CRFProcess* m_pcs)
{
	...
	if (this->ProcessIndex_CO2inLiquid == -1)
	{
		cout << "In the model exists dissolved gas but there is no dissolved C "
		        "in water defined. Please ad the mass transport for dissolved "
		        "CO2!"
		     << "\n";
		// system("Pause");
		exit(0);
	     ...
	     i++)
	{
		...
		// * density_liq[kg/m�] / (Molweight_CO2 [g/mol] * 1e-3 [kg/g])
		// TF commented out since we want to use the improved PointDuMux class
	...
	for (int k = 0; k < int(this->Phases.size()); k++)
	{
		if (k == 0)
		{
			...
			// i_ind[0] = m_pcs_fm->GetNodeValueIndex("VELOCITY_Y1")+1; // get
			// index of velocity
			// if(coordinateflag == 12)
			// i_ind[0] = m_pcs_fm->GetNodeValueIndex("VELOCITY_Z1")+1; // get
			// index of velocity
			// i_ind[1] = m_pcs_fm->GetNodeValueIndex("VELOCITY_Y1")+1; // get
			// index of velocity
			// if(coordinateflag == 22)
			// i_ind[1] = m_pcs_fm->GetNodeValueIndex("VELOCITY_Z1")+1; // get
			// index of velocity
			// i_ind[2] = m_pcs_fm->GetNodeValueIndex("VELOCITY_Z1")+1; // get
			// index of velocity
		}
		else
		{
			...
			// index of velocity
			// if(coordinateflag == 11)
			// i_ind[0] = m_pcs_fm->GetNodeValueIndex("VELOCITY_Y2")+1; // get
			// index of velocity
			// if(coordinateflag == 12)
			// i_ind[0] = m_pcs_fm->GetNodeValueIndex("VELOCITY_Z2")+1; // get
			// index of velocity
			// i_ind[1] = m_pcs_fm->GetNodeValueIndex("VELOCITY_Y2")+1; // get
			// index of velocity
			// if(coordinateflag == 22)
			// i_ind[1] = m_pcs_fm->GetNodeValueIndex("VELOCITY_Z2")+1; // get
			// index of velocity
			// i_ind[2] = m_pcs_fm->GetNodeValueIndex("VELOCITY_Z2")+1; // get
			// index of velocity
		}
		...
		for (long i = 0; i < (long)m_msh->ele_vector.size(); i++)
		{
			...
			{   // Configure Element for interpolation of node velocities to GP
				...
				// + temp.str();
				// temp.str(""); temp.clear(); temp << gc[1]; tempstring += "; "
				// + temp.str();
				// temp.str(""); temp.clear(); temp << gc[2]; tempstring += "; "
				// + temp.str();
		...
		// temp.str();
		//	string aus_file = path + "\\CheckVelocity_" + Phases[k] + "_" +
		// tempstring + ".csv";
		//	ofstream aus;
