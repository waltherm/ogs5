                                     ...
                                     const std::string& unique_name)
{
	...
	while (!new_keyword)
	{
		...
		if (line_string.find("$CONSTRAINED") != std::string::npos)
		{
			      ...
			      temp.constrainedDirection == ConstrainedType::GREATER))
			{
				std::cout << "No valid constrainedDirection for "
				          << FiniteElement::convertProcessTypeToString(
				                 temp.constrainedProcessType) << " (" << tempst
				          << ")" << std::endl;
				_isConstrainedST = false;
			}
			...
			if (_isConstrainedST) this->_constrainedST.push_back(temp);
			in.clear();
		}
	...
	if (m_msh)  // WW
	{
		...
		for (long i = 0; i < no_st; i++)
		{
			...
			{   // NB For climate ST, the source terms (recharge in this case)
				// will also be assigned directly to surface nodes
                      ...
                      FiniteElement::PrimaryVariable primaryVariable)
{
	...
	edge->SetNormalVector(elemNormalVector, edge_normal);  // Attenzione! This
	                                                       // only works if
	                                                       // nodes are numbered
	                                                       // counter-clockwise.
                                  ...
                                  std::vector<double>& node_value_vector)
{
	...
	// CB THMBM
	// this->getProcess()->CheckMarkedElement(); // CB added to remove bug with
                               ...
                               CNodeValue* cnodev)
{
	...
	{  // gas
		...
		/*if( m_st->pcs_type_name_cond == "MULTI_PHASE_FLOW")
		   mesh_node_number_conditional = cnodev->msh_node_number_conditional -
		m_pcs_this->m_msh->nod_vector.size();
		else*/
		mesh_node_number_conditional = cnodev->msh_node_number_conditional;
...
void GetNODHeatTransfer(double& value, CSourceTerm* st, long geo_node)
{
	...
	// axisymmetric 2D meshes geometry area is irrelevant
	// geo_area = 1.0;
	...
	{
		if (geo_node == st->st_node_ids[i]) break;
	}
        ...
        std::vector<double>& ply_nod_val_vector)
{
    ...
    if (st->getProcessDistributionType() == FiniteElement::LINEAR
            || st->getProcessDistributionType() ==
FiniteElement::LINEAR_NEUMANN) {
        st->InterpolatePolylineNodeValueVector(old_ply, st->DistribedBC,
ply_nod_val_vector);
...
void CSourceTermGroup::DistributeVolumeFlux(CSourceTerm* st,
                                            std::vector<long> const& nod_vector,
                                            std::vector<double>& nod_val_vector)
{
	double area;
...
std::string CSourceTerm::DirectAssign_Precipitation(double current_time)
{
	...
	if (start_pos_in_st < 0) osize = (long)m_pcs->st_node.size();
...
double CSourceTerm::GetAnalyticalSolution(long location)
{
	...
	for (i = 0; i < size; i++)
	{
		    ...
		    convertPrimaryVariableToString(this->getProcessPrimaryVariable()))
		{
			...
			if (this->getGeoType() == GEOLIB::GEODOMAIN) process_no = i;
		}
	}
	// Identify process
	if (process_no == 1)
	...
	if ((aktueller_zeitschritt - 1) % this->resolution == 0)
	{
		...
		{  // HS 04.2008
			...
			for (i = 0; i < (int)Node->getConnectedElementIDs().size(); i++)
			{
				...
				flux_area = Ele->GetFluxArea();  // Real thickness for a
				                                 // fracture
                               ...
                               double gamma)
{
	...
	*h_this = m_pcs_this->GetNodeValue(msh_node_number, nidx);
	...
	{
		*h_cond = *h_shifted = m_st->coup_given_value;  // coupled to fixed
		                                                // pressure head / or
