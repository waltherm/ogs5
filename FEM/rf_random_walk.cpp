	...
	*/
	...
	if (nnode == 4)
	{
		...
		for (int i = 0; i < nnode; ++i)
		{
			...
			// //FM_TEST
			// vy[i] =
			// m_pcs->GetNodeValue(theEle->GetNodeIndex(i),idy)/porosity;
			// vz[i] =
...
void RandomWalk::CheckBoundary3D(Particle* A, Particle* B)
{
	...
	else
	{
		...
		for (int j = 0; j < (long)m_msh->face_vector.size(); j++)
		{
			...
			// triangles
			if (n_ele->GetElementType() == MshElemType::QUAD) ntr = 2;
			...
			for (itr = 0; itr < ntr; itr++)
			{
				...
				if (fabs(f_buff) < DBL_MIN) continue;
...
{
	if (E->GetDimension() == m_msh->GetMaxElementDim()) return;
	double x[3], xx[3];
