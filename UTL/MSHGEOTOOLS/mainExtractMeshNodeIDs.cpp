...
int main(int argc, char* argv[])
{
	if (argc < 5)
	{
		std::cout << "program " << argv[0]
		          << " takes a *layered* mesh file and closed polylines from a "
		             "geometry and computes the surface mesh nodes within the "
		             "closed polyline" << std::endl;
		std::cout
		    << "Usage: " << std::endl
		    << argv[0]
	...
	// layer, mesh_ids);
	//	writeMeshNodes(mesh, mesh_ids, "MeshIDs.txt", "MeshNodesAsPoints.gli",
	// true);
	...
	//(*plys)[bounding_polygon_id]->isClosed()) {
	//		bounding_polygon_id++;
	//	}
	//
	...
	// GEOLIB::Polygon((*((*plys)[bounding_polygon_id]))), holes);
