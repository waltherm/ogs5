...
{
	string _infor =
	    "\nData pre-process tool for the finite element analysis of subsurface "
	    "flow problems.\n "
	    "By Wenqing Wang\n\n"
	    "Command: fem_toolkits -opt [Option] [File name with extension]\n"
	    "Option:\n"
	    "  1:  Generation OGS Neumman BC from the recharge data in GIS raster "
	    "file,\n"
...
int main(int argc, char* argv[])
{
	...
	if (argc > 1)
	{
		for (int i = 1; i < argc; i++)
		{
			...
			if (!(s_buff.find("-") != string::npos ||
			      s_buff.find("--") != string::npos))
			{
				file_name = s_buff;
			}
		}
