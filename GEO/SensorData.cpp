	...
	if (_step_size > 0)
	{
		if (((_end - _start) / _step_size) != data->size())
			std::cout << "Warning in SensorData::addTimeSeries() - Lengths of "
			             "time series does not match number of time steps."
			          << "\n";
	}
    ...
    SensorDataType::type time_series_name) const
{
	...
	{
		if (time_series_name == _vec_names[i]) return _data_vecs[i];
	}
    ...
    SensorDataType::type time_series_name) const
{
	for (size_t i = 0; i < _vec_names.size(); i++)
	{
		if (time_series_name == _vec_names[i]) return _data_unit_string[i];
	}
