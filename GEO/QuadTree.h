...
public:
	...
	bool addPoint(POINT const* pnt)
	{
		...
		if (!_is_leaf)
			for (size_t k(0); k < 4; k++)
				if (_childs[k]->addPoint(pnt)) return true;
		...
		if (_pnts.size() > _max_points_per_node) splitNode();
		return true;
	}
	...
	{
		if (max_depth < _depth) max_depth = _depth;
...
private:
	...
	QuadTree<POINT>* getNorthNeighbor() const
	{
		...
		if (north_neighbor == NULL) return NULL;
		if (north_neighbor->isLeaf()) return north_neighbor;
	...
	QuadTree<POINT>* getSouthNeighbor() const
	{
		...
		if (south_neighbor == NULL) return NULL;
		if (south_neighbor->isLeaf()) return south_neighbor;
	...
	QuadTree<POINT>* getEastNeighbor() const
	{
		...
		if (east_neighbor == NULL) return NULL;
		if (east_neighbor->isLeaf()) return east_neighbor;
	...
	QuadTree<POINT>* getWestNeighbor() const
	{
		...
		if (west_neighbor == NULL) return NULL;
		if (west_neighbor->isLeaf()) return west_neighbor;
	...
	void splitNode()
	{
		...
		h_ll[0] = _ll[0];
		h_ur[1] = _ur[1];
		_childs[1] = new QuadTree<POINT>(
		    h_ll, h_ur, this, _depth + 1, _max_points_per_node);  // north west
		...
		h_ur = _ur;
		h_ur[1] = mid_point[1];
		_childs[3] = new QuadTree<POINT>(
		    h_ll, h_ur, this, _depth + 1, _max_points_per_node);  // south east
	...
	bool needToRefine(QuadTree<POINT>* node)
	{
		...
		if (north_neighbor != NULL)
		{
			if (north_neighbor->getDepth() == node->getDepth())
				if (!north_neighbor->isLeaf())
				{
					if (!(north_neighbor->getChild(SW))->isLeaf()) return true;
					if (!(north_neighbor->getChild(SE))->isLeaf()) return true;
				}
		}
		...
		if (west_neighbor != NULL)
		{
			if (west_neighbor->getDepth() == node->getDepth())
				if (!west_neighbor->isLeaf())
				{
					if (!(west_neighbor->getChild(SE))->isLeaf()) return true;
					if (!(west_neighbor->getChild(NE))->isLeaf()) return true;
				}
		}
		...
		if (south_neighbor != NULL)
		{
			if (south_neighbor->getDepth() == node->getDepth())
				if (!south_neighbor->isLeaf())
				{
					if (!(south_neighbor->getChild(NE))->isLeaf()) return true;
					if (!(south_neighbor->getChild(NW))->isLeaf()) return true;
				}
		}
		...
		if (east_neighbor != NULL)
		{
			if (east_neighbor->getDepth() == node->getDepth())
				if (!east_neighbor->isLeaf())
				{
					if (!(east_neighbor->getChild(NW))->isLeaf()) return true;
					if (!(east_neighbor->getChild(SW))->isLeaf()) return true;
				}
		}
		return false;
	}
