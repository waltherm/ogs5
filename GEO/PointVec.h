 ...
 *name.
 * It also handles the deleting of points. Additionally, each vector of points
 *is identified by
...
public:
	 ...
	 * identical points.
	 * The size of the axis aligned bounding box multiplied with the value of
	 * rel_eps gives the
	...
	PointVec(const std::string& name, std::vector<Point*>* points,
	         ...
	         double rel_eps = std::sqrt(std::numeric_limits<double>::min()));
	 ...
	 * ownership.
	 * If the given point is already included in the vector, the point will be
	 * destroyed and
