...
void Station::addProperty(std::string pname, double (*getFct)(void*),
    ...
    : Station(x, y, z), _zCoord(0), _depth(0), _date(0)
{
	...
	addProperty("depth", &StationBorehole::getDepth,
	            &StationBorehole::setDepth);
                                     ...
                                     StationBorehole* borehole)
{
	...
	if (readStratigraphyFile(path, data))
	{
		...
		//incomplete
		//(Borehole " << borehole->_name << ", Layer " << (i+1) << "
		// missing).\n";
		//
		//			return 0;
		//		}
                              ...
                              StationBorehole* borehole)
{
	    ...
	    4) /* check if there are enough fields to create a borehole object */
	{
		if (fields.front().compare(borehole->_name) ==
		    0) /* check if the name of the borehole
		          matches the name in the data */
		{
			fields.pop_front();
                                                ...
                                                std::string date)
{
	...
	if (date.compare("0000-00-00") != 0) station->_date = xmlDate2int(date);
	return station;
}
...
void StationBorehole::addSoilLayer(double x, double y, double z,
                                   const std::string& soil_name)
{
