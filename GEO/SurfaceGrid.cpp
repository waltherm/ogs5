    ...
    : AABB(sfc->getAABB()), _triangles_in_grid_box(NULL)
{
	...
	if (_n_steps[2] * _n_steps[1] * _n_steps[0] > 1000) return;
		...
		for (std::size_t j(0); j < _n_steps[1]; j++)
		{
			for (std::size_t i(0); i < _n_steps[0]; i++)
			{
				const std::size_t cell_id(k * _n_steps[0] * _n_steps[1] +
				                          j * _n_steps[0] + i);
				if (_triangles_in_grid_box[cell_id].empty()) continue;
...
void SurfaceGrid::writeTrianglesInGridCell(std::size_t i, std::size_t j,
                                           std::size_t k,
