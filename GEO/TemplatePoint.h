...
public:
	 ...
	 * double sqrNrm2 = point[0] * point[0] + point[1] * point[1] + point[2] +
	 * point[2];
	 * \endcode
	 */
...
template <class T>
TemplatePoint<T>::TemplatePoint(T x1, T x2, T x3)
    : GeoObject()
{
	_x[0] = x1;
	_x[1] = x2;
...
template <class T>
TemplatePoint<T>::TemplatePoint(T const* x)
    : GeoObject()
{
...
template <class T>
TemplatePoint<T>::TemplatePoint(TemplatePoint<T> const& src)
    : GeoObject()
{
	for (size_t k(0); k < 3; k++)
		_x[k] = src._x[k];
