 ...
 *may be empty).
 * It may include a name, a stratigraphy (only for the derived class
 *StationBore),
 ...
 *this class (this is entirely optional!).
 * Certain functions in the GUI allow you to modify aspects of the visualisation
 *based on these
 * properties (e.g. filtering operations such as "display only boreholes drilled
 *after 1990 with a
 * depth between 400-800m").
 * To make use of this functionality you need to define properties using the
 *"Station::addProperty()"-method.
 * Parameters for this function include the name of the property as well as a
 *method to read and write the
 * value of the associated variable (see documentation for "addProperty" for
 *details). Furthermore, these read
 * and write-functions need to be actually implemented as static functions to
 *avoid casting problems with the
 * function pointers used to dynamically connect the GUI functionality to the
...
public:
	 ...
	 * This definition consists of a name for the property as well as a function
	 *pointer to a method to
	 * read and to write this variable.
	 * Due to inheritance these function pointers only work correctly if the
	 *read and write functions are
	 * implemented as static functions, i.e. both the read and the write functin
	 *get a void-pointer to the
	 * actual station-object. This pointer is then casted to the correct class
	 *and the respective value can
	 * then be read or written dynamically from that object. It is highly
	 *recommended to define both the
	 * read and write function as protected because it does not actually make
	 *sense for these functions to be
	 * static except in the context of function pointers. Please refer to the
	 *examples below, i.e. the getX
	 * and setX methods.
	 * \param pname The name of the property.
	 * \param get A function pointer to a static read function for the variable
	 *referred to by pname
	 * \param set A function pointer to a static write function for the variable
	 *referred to by pname
	 * \return
	 */
	void addProperty(std::string pname,
	                 double (*get)(void*),
	                 void (*set)(void*, double));
...
protected:
	 ...
	 * (i.e. the x-coordinate of the station). It is implemented as a static
	 *method to avoid casting issues
	 * related to the function pointer associated with this function. Therefore,
	 *this function needs to be
	 * called "getX((void*)this);". It is highly recommended to define this
	 *function as protected because it
	 * does not actually make sense for these functions to be static except in
	 *the context of function pointers.
	 * \param stnObject A pointer to the station object for which the
...
public:
	 ...
	 * that the beginning of the first layer is identical with the position of
	 * the borehole. For each
	 * layer following the beginning is already given by the end of the last
	 * layer. This also saves
	 * a seperate entry in the profile vector for the end of the borehole which
	 * in the given notation
	 * is just the coordinate given for the last soil layer (i.e. the end of
	 * that layer).
	 */
