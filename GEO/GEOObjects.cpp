...
const PolylineVec* GEOObjects::getPolylineVecObj(const std::string& name) const
{
	size_t size(_ply_vecs.size());
	for (size_t i = 0; i < size; i++)
		if (_ply_vecs[i]->getName().compare(name) == 0) return _ply_vecs[i];
...
const SurfaceVec* GEOObjects::getSurfaceVecObj(const std::string& name) const
{
	size_t size(_sfc_vecs.size());
	for (size_t i = 0; i < size; i++)
		if (_sfc_vecs[i]->getName().compare(name) == 0) return _sfc_vecs[i];
...
void GEOObjects::optimiseObjects(const std::string& name,
                                 const GEOLIB::AABB& bb)
{
	const SurfaceVec* sfcVec = getSurfaceVecObj(name);
	std::vector<Surface*>* sfc_vec =
	    const_cast<std::vector<Surface*>*>(getSurfaceVec(name));
...
int GEOObjects::exists(const std::string& geometry_name) const
{
	size_t size(_pnt_vecs.size());
	for (size_t i = 0; i < size; i++)
		if (_pnt_vecs[i]->getName().compare(geometry_name) == 0) return i;
