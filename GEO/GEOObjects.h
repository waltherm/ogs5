 ...
 *objects.
 * Said objects are Points, polylines, Surfaces and Stations and they are stored
 *in
 * vectors (arrays) which are identified by a unique name.
 * For a hierarchical definition, surfaces are bounded by polylines and
 *polylines
 * are defined by points. Therefore, a vector of surfaces references a vector
 *polylines
 * and a vector of polylines references a vector of points, respectively. For
 * identification purposes, all of these vectors have the same name, i.e. the
 *polyline-
 * vector named "aaa" references a point vector "aaa". However, this name
 *("aaa") is
 * unique among all the vectors of the same class, i.e. there exists only one
 *point-
 * vector with this name, etc.
 * Note: The fact that vectors are uniquely named and the same name is assigned
 *to
 ...
 * E.g. for points these methods are "addPointVec(name)", "getPointVec(name)"
 *and
...
public:
	 ...
	 * the provided name.
	 * The pointers are managed by the GEOObjects, i.e. GEOObjects will delete
	 * the Points at the
	 * end of its scope.
	 ...
	 * within the internal vector.
	 * In case the ids are not needed it is possible to give a NULL pointer
	 * (default value).
	 ...
	 * PointVec
	 * object takes care about deleting the point. If the point already exists
	 * within the
	 ...
	 * with provided name.
	 * the pointers are managed by the GEOObjects, i.e. GEOObjects will delete
	 * the Polylines at the
	 * end of its scope
	 * \param polylines the vector with polylines
	 * \param name the name of the internal PolylineVec
	 * \return true if the polylines are appended, false if the PolylineVec with
	 * the
	 * corresponding name does not exist
	 * */
...
protected:
	 ...
	 * vectors and creates a
	 * new name if this is not the case. The new name is then simply "name + x",
	 * where x>1 is
	 * the smallest number that creates a unique name (i.e. "name-2", "name-3",
	 * etc.)
	 * \param name Original name of the list, this name might be changed within
