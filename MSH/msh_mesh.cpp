...
long CFEMesh::GetNODOnPNT(const GEOLIB::Point* const pnt) const
{
	...
	if (!useQuadratic) return node_id;
                          ...
                          double eps)
{
	...
	if (!automatic) search_radius = eps;
...
bool CFEMesh::NodeExists(size_t node)
{
	...
	for (size_t i = 0; i < no_nodes; i++)
		if (node == (size_t)Eqs2Global_NodeIndex[i]) return true;
	return false;
}
                                   ...
                                   std::vector<long>& vec_elements)
{
	...
	for (i = 0; i < (long)nodes_on_sfc.size(); i++)
	{
		     ...
		     j++)
		{
			...
			if (ele_vector[l]->selected == 0) vec_elements.push_back(l);
