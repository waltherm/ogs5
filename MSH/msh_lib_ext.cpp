...
// kg44 quick fix to compile PETSC with version PETSCV3.4
	...
	if (rc)
	{
		if (mrank == 0) printf("-->Reading ASCII mesh file ...");
	...
	{
		MPI_File_close(&fh);
		if (mrank == 0) printf("-->Reading binary mesh file ...");
                 ...
                 GEOLIB::GEOObjects* geo_obj, string* unique_name)
{
	...
	{
		MPI_Finalize();
		if (mrank == 0) printf("! File %s does not exist.", &fname_new_base[0]);
		exit(0);
	}
	...
	*/
	...
	for (size_t e = 0; e < e_size; e++)
	{
		...
		for (int i = elem->nnodes; i < elem->nnodesHQ; i++)
		{
			...
			if (!done) node->getConnectedElementIDs().push_back(e);
		}
	}
