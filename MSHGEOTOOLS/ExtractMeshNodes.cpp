                                        ...
                                        const GEOLIB::Polygon& polygon)
{
	...
	for (size_t j(0); j < msh_nodes.size(); j++)
		if (msh_nodes[j]->Interior())
		{
			...
			if (polygon.isPntInPolygon(pnt)) node_indices.push_back(j);
		}
	// write data
    ...
    std::vector<GEOLIB::PointWithID>& nodes_as_points) const
{
	...
	if (polyline.isClosed()) number_of_ply_pnts--;
                                                ...
                                                const GEOLIB::Polygon& polygon)
{
	...
	{
		GEOLIB::Point pnt(msh_nodes[j]->getData());
		if (polygon.isPntInPolygon(pnt)) node_ids.push_back(j);
	}
	...
	for (size_t k(0); k < n_nodes; k++)
	{
		...
		for (size_t j(0); j < n_mesh_elem_ids; j++)
		{
			...
			for (size_t i(0); i < n_of_nodes_of_element && found; i++)
			{
				...
				if (it == node_ids.end()) found = false;
			}
			if (found) area += msh_elem[mesh_elem_ids[j]]->calcVolume();
		}
		areas[k] = area / 3.0;
	}
	...
	for (size_t j(0); j < msh_nodes.size(); j++)
	{
		...
		if (bounding_polygon.isPntInPolygon(pnt))
		{
			...
			for (size_t k(0); k < n_holes && is_not_in_hole; k++)
				if ((holes[k])->isPntInPolygon(pnt)) is_not_in_hole = false;

			if (is_not_in_hole) node_ids.push_back(j);
		}
	}
	std::sort(node_ids.begin(), node_ids.end());
