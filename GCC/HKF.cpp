...
Helgeson, H. C., 1992.
    Calculation of the Thermodynamic Properties of Aqueous Species at
High-Pressures and Temperatures-
    Effective Electrostatic Radii, Dissociation-Constants and Standard Partial
Molal Properties to
...
and Transport-
    Properties of Aqueous Species at High-Pressures and Temperatures - Aqueous
Tracer Diffusion-
...
int HKF::HKF_OGS_loadparam(std::string species_name0, int& type, double& charge,
                           double param[][4])
{
	int i, j;
...
int HKF::HKF_OGS_calc(double T, double P, double& G, double& H, double& S,
...
double HKF::HKFcalc(double T, double P, int ghs, std::string name, int sub,
                    int type)
{
	if (ghs > 2 || ghs < 0) exit(0);
	SpeciesData ion;
...
int HKF::load_param(vector<SpeciesData>& spec, vector<int> name_type,
                    vector<int> phase_type)
{
...
int HKF::HelgesonEquation(double T, double P, vector<SpeciesData>& spec)
{
	...
	double a0, b0, c0 /*, Tmax*/;  // gases and minerals parameters from
	                               // database
				...
				case 4:
					...
					spec[i].G = dG - dS * (T - Tr) +
					            ...
					            (-1.0 * c0 - b0 * T * Tr * Tr) * (T - Tr) *
					                (T - Tr) / (2.0 * T * Tr * Tr) +
					            dV * (P - Pr) * 0.1 / 4.18;  // volume, pressure
					                                         // and energy unit
					                                         // convert factor
					spec[i].H =
